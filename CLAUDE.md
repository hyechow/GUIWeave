...
```bash
# iPhone(默认平台)
uv run python -m policy_expr.core.runner "打开微信并进入通讯录" --auto-continue --hud --supervisor milestone
...
AGENT_PLATFORM=browser uv run python -m policy_expr.core.runner "点击右上角设置按钮" --auto-continue
...
- 搬运模块时同时 grep `policy_expr.X` 与 `from policy_expr import X` 两种导入形式,并查 `__file__`/`parents[N]` 深度假设。
