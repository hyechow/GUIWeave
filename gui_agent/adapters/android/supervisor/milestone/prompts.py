...
- ⚠️ 操作滚轮选择器（时间 / 日期 picker，中间高亮行=当前选中值）：对比当前值与目标值——差很多格时指令「在<小时/分钟>列滚动以快速接近<目标值>」，差几格时指令「在<小时/分钟>列滚动一点，把<目标值>滚到中间高亮行」逐格精调，冲过头就反向。调 picker 往往要连续多次滚动，**只要选中值在朝目标变化就是有进展、不要判为卡住**；禁止指令「点击 picker 里的数字」（点击不生效、也不需要点击确认）
...
- drag_column / drag_current_value / drag_target_value：Android 的滚轮 picker（时间/日期选择器）用普通 scroll 逐步操作即可（见上「操作滚轮选择器」），这三个字段一律留空
"""
...
## 决策规则
- ⚠️ 若失败子目标是操作滚轮 picker（时间/日期选择器）：正确的「换路径」是调整滚动的方向与幅度——离目标远用 medium 快速接近、近用 small 逐格精调、冲过头就反向，**继续滚动逼近目标值**；只要选中值在朝目标变化就是有进展，不算卡住。**绝不要改成「点击 picker 里的数字」或去点别的元素**——点 picker 零效果，会被判为操作无效而中止任务（上面「跳出该元素找新按钮」的规则不适用于 picker）
