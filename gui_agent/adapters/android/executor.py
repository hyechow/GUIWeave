...
``AndroidExecutor.execute`` matches the iphone ``ActionExecutor.execute`` signature
...
Coordinates ``action.x`` / ``action.y`` are normalized 0-1000 over the screen; they
are denormalized to DEVICE PIXELS via the device's ``viewport_size`` (the cached
``window_size()``). No mirror-window mapping, no retina halving — the same pixel
goes straight to ``adb shell input``. Intentionally SIMPLE: NO YOLO snap, NO OCR,
NO picker.

Only the neutral action vocabulary is dispatched (tap / type / clear_text /
press_enter / scroll / drag / home / stop). Any leaked iphone-only fields
(picker_* target_area, value_direction, drag_column/steps) are simply never read
here, so they are harmless — same safe-ignore contract as the browser executor.
"""
...
class AndroidExecutor:
    ...
    def __init__(self, session):
        # ``session`` is an AndroidSession; the device lives on ``.client``.
    ...
    def _denorm(self, ax: float, ay: float) -> tuple[float, float]:
        """Normalized 0-1000 -> device pixels, clamped inside the screen."""
    ...
    def prepare_frame(self, png_bytes: bytes) -> None:
        """No-op: the android adapter is vision-only (no YOLO/OCR tap snapping).
        The runner submits this to a background thread each turn; iphone uses it to
        precompute snap data — android has nothing to precompute."""
        return None
    ...
    def execute_scroll(self, action, *, ticks: int = 0, delta_px: int = 0) -> None:
        ...
        android scroll is a deterministic swipe."""
        client = self._client()
    ...
    def execute(
        ...
        if action.action_type in ("tap", "click") and action.x is not None and action.y is not None:
            ...
            # Replace existing contents: clear-then-type the new text.
...
# Map the schema's coarse ScrollAmount onto AndroidDevice.scroll amount units.
# (AndroidDevice swipes amount * 200px; 3/5/9 ≈ small/medium/large on a tall panel.)
