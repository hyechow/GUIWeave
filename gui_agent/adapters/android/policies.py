...
## 滚轮选择器（时间 / 日期 picker）
- 形态：一列或多列上下滚动的轮子（如闹钟的「上午/下午 · 小时 · 分钟」）。**中间高亮的那一行就是当前选中值。**
- 改值只能靠滚动，**分两段**：离目标还差很多格时用 amount=medium 快速接近；差几格时改用 amount=small 一格格精调，把目标值停到中间高亮行。别一直用 small（远距离挪不到），也别一直用 medium（近了会冲过头）；冲过头就反向滚回来。
- scroll 的 x/y 锚点必须落在要滚的那一列上（小时列、分钟列各自的 x），不要落在屏幕最上方或最下方。
- **绝不要 tap picker**（包括点中间那个已选中的值）——picker 的值只靠滚动设定，点击既不生效、也不需要点击来「确认」；要选某个值，只能把它滚到中间高亮行。
