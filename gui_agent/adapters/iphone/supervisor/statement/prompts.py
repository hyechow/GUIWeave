...
# Checker prompt is assembled per statement.kind: SINGLE_CHECKER_PROMPT is the
...
SINGLE_CHECKER_PROMPT = load_prompt_text("task.statement.iphone.checker")

# ── Per-kind checker sections (only the relevant one is injected) ──────────
_CHECK_SECTION_NAVIGATION = load_prompt_text("context.statement.iphone.check.navigation")

_CHECK_SECTION_FILTER = load_prompt_text("context.statement.iphone.check.filter")

_CHECK_SECTION_ACTION = load_prompt_text("context.statement.iphone.check.action")

_CHECK_SECTION_COLLECTION = load_prompt_text("context.statement.iphone.check.collection")

# 连续调值类（completion_strategy=repeat_until_satisfied）专用补充段。叠加在 kind 段之上，
# 由 run_checker 在 statement.is_converge 时追加。核心：当前值以滚轮中心带为准（摘要框滞后），
# 并强制输出当前值/目标值，作为连续操作的进展传感器（供 planner 算步长、供 stuck 判值停滞）。
_CHECK_SECTION_CONVERGE = load_prompt_text("context.statement.iphone.check.converge")
...
LOOP_FRAME_PROMPT = load_prompt_text("task.statement.iphone.loop_frame")

PLAN_PROMPT = load_prompt_text("task.statement.iphone.planner")

LOOP_SCROLL_PROMPT = load_prompt_text("task.statement.iphone.loop_scroll")

REPLAN_PROMPT = load_prompt_text("task.statement.iphone.replanner")

# ── Bundle the above into the neutral StatementPrompts seam ──────────────────
# This iPhone-flavored bundle owns the injection boundary; the model-visible
# prompt bodies live under gui_agent/prompts/tasks/statement/iphone/.
