...
11. ⚠️ 分析类目标里的「计算/汇总/求和/统计总额/算出总数/对比」是对**已采集数据的纯运算**，由系统输出环节自动完成，**禁止**为它单独生成子目标（如「汇总计算总支出金额」「算出总和」「统计共多少条」）。
   ...
   - ⚠️ **哪怕起始截图上已经显示「支出¥X / 收入¥Y / 收支统计」之类的汇总数字，也绝不能把「读取这个屏上摘要」当作终点或验收**：那个摘要几乎总是**整月/全量**范围、不是目标日期区间，且分析任务要的是**逐条明细**（供按范围汇总）。分析类「花多少钱 / 金额 / 统计」任务**必须**以 kind=collection + completion_strategy=scroll_until_boundary 的滚动采集明细收尾；**不要**因为屏上已有现成数字，就把采集步退化成「筛选 + 读摘要」的 action 子目标。
"""
