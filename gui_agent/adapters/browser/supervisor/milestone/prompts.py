...
- ⚠️ 搜索式下拉框只按【当前截图】判定选定状态，绝不沿用历史轮次的展开状态。判据=该输入框下方是否
  紧贴**无字段标签**的候选选项行（候选浮层盖在表单之上、会遮住下一个字段的标签，且此时输入框通常带
  焦点高亮边框；兄弟字段自带字段标签和「请选择…」占位符、也常带搜索图标，别把它认成候选列表）：
  （展开）有候选浮层 → 框内文字只是过滤词、**尚未选定**（移开焦点会被清空），该字段计入
  missing_evidence（写明需点击候选选项完成选定）
  （收起）无候选浮层、下一个字段的标签完整可见，且输入框显示完整目标文本 → **已选定，该字段判定
  结束**（其余字段照常逐一核对值，不符的照常计入 missing_evidence）。未选定的过滤词失焦即被清空，
  文本能留下本身就是候选已点过的证据；选定后输入框仍带搜索图标、样式与过滤态相同、没有额外的
  「已选中」标志——不得以此质疑、不得写「需确认是否已点选」，不得要求重新输入或重新点击候选
  拿不准浮层是否存在时：下一个字段的标签完整可见 → 按已收起处理；标签被遮挡 → 按展开处理
- ⚠️ 不是所有操作都有确认/提交按钮：很多控件**即选即生效**（下拉选定、开关/单选切换）。验收看**状态本身**
  （字段已显示目标值、开关已处于目标态、结果已更新）；截图中不存在确认/提交按钮时，missing_evidence
  **不得**要求「点击确认/执行/提交按钮」或「提交后的成功提示」——只写真正缺失的可观察状态
- 只看可观测事实，不要凭感觉判断
...
- ⚠️ 搜索式下拉框（带搜索框、点开弹出候选列表）：第一步先在【当前截图】里确认该字段下方有没有候选浮层
  （紧贴输入框、无字段标签的选项行，会遮住下一个字段的标签；带自己标签和「请选择…」占位符的是下一个
  表单字段，不是浮层）。checker 的 reason/missing_evidence 描述的可能是过时状态，浮层有无只信截图：
  （0）截图中**无候选浮层**、且该下拉搜索框内已显示完整目标文本 → 该下拉框**已完成**：禁止点击它、
  禁止重新输入、禁止重选——未选定的过滤词失焦即被清空，文本能留下=候选已点过；即使 missing_evidence
  仍在催这个字段（那是误判），也直接处理下一个未完成字段
  截图中候选浮层展开时，先解决这个下拉框，不要跳去操作其他字段：
  ...
  输出前自检：如果指令是「点击候选/选项 X」，先确认截图中真的有候选浮层（命中（0）则禁止点击），且 X 必须等于当前下拉搜索框中已输入的非占位文本，或等于子目标要求的目标原文；否则不要点击 X，改为「输入 <目标原文>」过滤
