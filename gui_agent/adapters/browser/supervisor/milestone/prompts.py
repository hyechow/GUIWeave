...
- ⚠️ 不是所有操作都有确认/提交按钮：很多控件**即选即生效**（下拉选定、开关/单选切换）。验收看**状态本身**
  ...
  **不得**要求「点击确认/执行/提交按钮」或「提交后的成功提示」——只写真正缺失的可观察状态。
  如果页面没有这类按钮，就不要把“未点击按钮/未提交”当作缺失证据，也不要在 reason/summary 里把
  “未看到确认/提交/执行按钮”当成问题；只能基于目标值、目标状态或结果是否可见来判断
- 输出自检：除非截图中确实存在确认/提交/执行按钮且当前子目标明确要求点击它，否则 reason、summary、
  missing_evidence 都不要出现「确认按钮」「提交按钮」「执行按钮」这类缺失按钮表述
- 只看可观测事实，不要凭感觉判断
...
- ⚠️ 搜索式下拉框（带搜索框、点开弹出候选列表）：第一步先在【当前截图】里确认该字段附近有没有候选浮层。
  候选浮层通常是紧贴当前输入框的一组选项行，常无字段标签、可覆盖相邻内容；相邻表单字段通常有自己的
  字段标签/占位符/边框，不是候选。checker 的 reason/missing_evidence 可能描述的是过时状态，浮层有无
  必须回到截图核对：
  （0）截图中无候选浮层、且该单选下拉框内已显示完整目标文本 → 通常视为该下拉框已完成：不要为了
  “确认是否选中”而重新点击、重新输入或重选；即使 missing_evidence 仍在催这个字段，也优先处理下一个
  当前截图中确实未完成的字段。若截图明确显示这是可自由输入文本、还需应用/提交，或存在未确认标记，
  则按该 UI 的可见状态处理
  ...
  输出前自检：如果指令是「点击候选/选项 X」，先确认截图中真的有候选浮层（命中（0）则不要点击），且 X 必须等于当前下拉搜索框中已输入的非占位文本，或等于子目标要求的目标原文；否则不要点击 X，改为「输入 <目标原文>」过滤
