...
⚠️ 表格/Grid 的列筛选、后台列表筛选、站内搜索框等“填写后需提交/应用”的 UI：输入框里出现目标文本不等于筛选已生效。Search/Apply/Filter/Submit 等按钮可能是常驻控件，按钮仍可见本身不能证明未应用；只有在没有提交动作、没有列表/计数刷新证据、或可见结果与筛选条件不匹配时，才判 in_progress，并在 missing_evidence 写明需要提交/应用筛选。
⚠️ 使用结果计数（如 "X records found"、分页总数）作为验收证据前，必须先确认它是应用目标筛选后的计数：可见结果行应与筛选条件一致，或页面有明确的已应用筛选状态/刷新后的结果。若可见行中仍有任一条不符合筛选条件，不能把该计数当作目标计数；若计数已刷新且可见结果均符合条件，即使 Search/Apply 按钮仍常驻可见，也可以判 done。
