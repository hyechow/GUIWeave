...
- ⚠️ 搜索式下拉框：输入框内有文字但下方候选列表仍展开 = 那只是过滤词、**尚未选定**（移开焦点会被清空）。
  该字段必须计入 missing_evidence（写明需点击候选选项完成选定）；列表收起且字段显示已选值才算已设置
...
- ⚠️ 搜索式下拉框（带搜索框、点开弹出候选列表）按当前展开状态处理。只要候选列表展开，先解决这个下拉框，不要跳去操作其他字段：
  ...
  输出前自检：如果指令是「点击候选/选项 X」，X 必须等于当前下拉搜索框中已输入的非占位文本，或等于子目标要求的目标原文；否则不要点击 X，改为「输入 <目标原文>」过滤
