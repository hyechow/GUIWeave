...
def _effective_filter_intents(
    ...
    declared = [
        ...
        for control, value in (statement.target_values or {}).items()
...
def observed_filter_intent(
    ...
    if not applied_filters or not form_controls:
        return None
    if not statement.target_values and runtime_intent is None:
        return None
    desired_values = {
        tuple(sorted(_value_tokens(str(value))))
        for value in (statement.target_values or {}).values()
        if str(value).strip()
    }
    ...
    resolved = [
        ...
        if _effective_filter_intents(statement, candidate)
        != _effective_filter_intents(statement, None)
    ]
...
def _matched_applied_filter_labels(
    ...
    if not applied_filters:
        return set()
    intents = _effective_filter_intents(statement, runtime_intent)
    if not intents:
        return set()
...
def filter_chips_clean(
    ...
    matched_labels = _matched_applied_filter_labels(
        applied_filters, statement, runtime_intent
    )
...
def filter_residual_labels(
    ...
    declared_controls = tuple(
        ...
        for intent in _effective_filter_intents(statement, runtime_intent)
    )
    return [
        label
