"""Feasibility Guard: runtime statement-feasibility judgment (the goal-level stuck diagnosis).

When the agent is STUCK on a statement, this asks a different question from the action-level
replanner: is the statement's GOAL even achievable here — does the required UI control exist? —
or is it just that the action/query was wrong? If the required control is observed ABSENT, the
statement is infeasible and the run should kick back to the orchestrator with a directive rather
than retry forever / give up.

Reliability recipe (validated offline on task-113, robust to wrong knowledge — see
scripts/statement_feasibility_113.py and the stuck-diagnosis-two-mechanisms memory):
...
_SYSTEM = load_prompt_text("task.statement.feasibility")


class FeasibilityVerdict(BaseModel):
    """Feasibility Guard output: is the stuck statement feasible, and (if not) the kick-back directive.

    kickback = statement 函数抛出的类型化异常：``dead_route``/``required_route`` 是异常的结构化
...
def judge_feasibility(
    ...
    *,
    ...
    """Judge whether the stuck statement is executable on the current page.
    ...
    human = (
        f"卡住的 statement 目标(success_condition):\n{statement_goal}\n\n"
