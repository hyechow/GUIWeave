...
def route_identity_evidence(statement: StatementContract, observation: Observation) -> str:
    """Describe a route identity only when the statement names the same machine token."""
    ...
    declared = {
        token.casefold()
        for token in _IDENTITY_TOKEN_RE.findall(statement_text)
        if any(ch.isdigit() for ch in token)
    }
    route_tokens = {
        token.casefold()
