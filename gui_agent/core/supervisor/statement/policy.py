...
from .stuck import StatementStuckMixin
...
class StatementSupervisorPolicy(
    ...
    def set_app_knowledge(
        ...
        # KnowledgeSelector micro-decision picks section ids per (statement, page) — cached,
        # so it only fires on page/statement changes. Falls back to the full `elements`
        # blob when absent.
    ...
    @property
    def _monitor(self):
        return self._rt.monitor

    @property
    def _constraint_ledger(self):
        return self._rt.constraint_ledger

    @property
    def _execution_contract(self):
        return self._rt.execution_contract

    @_execution_contract.setter
    def _execution_contract(self, value):
        self._rt.execution_contract = value
    ...
    def _last_check(self):
        return None if self._statement_rt is None else self._statement_rt.last_check

    @_last_check.setter
    def _last_check(self, value) -> None:
        if self._statement_rt is not None:
            self._statement_rt.last_check = value

    @property
    def _done_check(self):
        return None if self._statement_rt is None else self._statement_rt.done_check

    @_done_check.setter
    def _done_check(self, value) -> None:
        if self._statement_rt is not None:
            self._statement_rt.done_check = value

    @property
    def _last_page_identity(self) -> str:
        return self._rt.last_page_identity

    @_last_page_identity.setter
    def _last_page_identity(self, value: str) -> None:
        self._rt.last_page_identity = value

    @property
    def _scroll_count(self) -> int:
        return self._rt.scroll_count

    @_scroll_count.setter
    def _scroll_count(self, value: int) -> None:
        self._rt.scroll_count = value

    @property
    def _early_feasibility_probed(self) -> bool:
        return self._rt.early_feasibility_probed

    @_early_feasibility_probed.setter
    def _early_feasibility_probed(self, value: bool) -> None:
        self._rt.early_feasibility_probed = value

    @property
    def _skip_initial_check(self) -> bool:
        return self._rt.skip_initial_check

    @_skip_initial_check.setter
    def _skip_initial_check(self, value: bool) -> None:
        self._rt.skip_initial_check = value

    @property
    def _target_acquire(self):
        return self._rt.target_acquire
    ...
    def begin_statement(
        ...
        *,
        ...
        self._statement_rt = StatementRuntimeState(
            ...
            execution_contract=ExecutionContract.from_statement(contract),
            task_type=task_type,
    ...
    def resume_statement(
        ...
        *,
        ...
        last_checker = next(
            (turn.checker for turn in reversed(invocation) if turn.checker),
            None,
        )
        if last_checker is not None:
            try:
                self._last_check = _SingleCheckResult.model_validate(last_checker)
            except Exception:
                self._last_check = None

    def reset_for_return_retry(self, new_contract) -> None:
        """Return-contract tighten: reuse instance, recompile contract, keep constraints."""
        from gui_agent.core.run.execution_signals import ExecutionContract
        self._rt.reset_for_return_retry(
            new_contract,
            ExecutionContract.from_statement(new_contract),
        )
    ...
    def _collection_completion_decision(
        ...
        *,
        ...
        return self._execution_coordinator.decide(
            execution_contract_for(statement, self._execution_contract),
            [claim(
                "collection.coverage",
                "complete",
    ...
    def _advance_from_collection_controller(
        ...
        *,
        ...
        scope = self._scope_for(statement, observation)
    ...
    def _run_single_turn(
        ...
        # checker call on the statement hand-off. One-shot.
        if self._skip_initial_check:
            ...
            synthetic = _SingleCheckResult(
                ...
                reason=f"刚进入子目标「{statement.name}」，默认未完成",
                summary="",
            )
            self._last_check = synthetic
            self._last_page_identity = ""
            return self._plan_single(statement, synthetic, observation, scoped_history)

        if statement_history and statement_history[-1].action_decision:
            last_action = statement_history[-1].action_decision.action
        ...
        # is absent from ``statement_history`` by design. Bridge only the immediately preceding
        # action of this statement into the response update; older rows/scopes remain isolated.
        response_history = statement_history
        if (
            history
        ...
        # ordinary statement actions after the target enters the viewport.
        acquire = None
        ...
        if (
            ...
            and statement.completion_strategy == "read_once"
        ...
        if statement.completion_strategy == "react_until_collected":
        ...
        # An action that ACTUALLY satisfied the statement advances even when the verifiers
        ...
        # statement the action already completed. See logs/.../android/20260611_085000.
        if check.status == "done":
            final_read = None
            if statement.kind in {"collection", "verification"} and self.task_type != "action":
                read_inst = check.read_instruction or _default_read_instruction(statement)
                final_read = _ctx(statement, read_inst)
            return self._advance(
                statement,
                observation,
                history,
        ...
        if self._observe_only:
            return SupervisorStep(
                ...
                execution_scope=execution_scope,
                **_ctx(statement, check.read_instruction),
            )
        ...
        if post_completion.next == "act" and check.status == "stuck":
            return self._handle_stuck(
                statement, check, check.read_instruction, observation, scoped_history
            )
        ...
        # statement is NOT done (checked above) → route straight into replan. Catches "screen
        # changed but to the wrong element" (e.g. 搜索框 tap hit 转账 tab), which SimStuck can't.
        last_tv = statement_history[-1].target_verify if statement_history else None
        ...
        last_turn = statement_history[-1] if statement_history else None
        if (
        ...
        prev_action = (
            statement_history[-1].action_decision.action
            if statement_history and statement_history[-1].action_decision
            else None
        )
        ...
        if statement.is_iterative:
            ...
            if sim_stuck is not None:
                ...
                return self._handle_stuck(
                    statement, sim_stuck, check.read_instruction,
            ...
            if stall is not None:
                return self._handle_stuck(
                    statement, stall, check.read_instruction,
                    observation, scoped_history,
                    page_changed=False,
                    prev_page_id=prev_page_id, current_page_id=current_page_id,
                )
        ...
        rep_stuck = (
            ...
            else self._monitor.check_instruction_repetition(scoped_history, statement.id)
        )
        ...
        if sim_stuck or rep_stuck:
            ...
            return self._handle_stuck(
                statement, progress, check.read_instruction,
                observation, scoped_history,
    ...
    def _plan_single(
        ...
        persistence = persistence_assessment or assess_persistence(
            statement, history, scope=execution_scope, current_surface=current_surface_id
        )
        mutation_required = bool(statement.kind == "action" and statement.target_values)
        mutation_subject = (
            resolve_mutation(
                statement,
                observation,
        ...
        if mutation_subject is not None and mutation_subject.status in {
            ...
            writes_complete = bool(statement.target_values) and all(
        ...
        atomic_role, action_family = action_metadata(plan, statement)
        mutation_authorization = None
        if mutation_owned_plan and atomic_role == "write" and mutation_subject is not None:
            mutation_authorization = authorize_mutation(statement, mutation_subject)
    ...
    def _run_loop_turn(
        ...
        if self._observe_only:
            with _Timer(self._timings, self._timings_order, "loop_check", self._token_usage):
                frame = self._loop_check(statement, observation, scoped_history)
            self._last_check = None
            read_inst = (
                None
                if self.task_type == "action"
                else frame.read_instruction or _default_read_instruction(statement)
            )
            if frame.should_stop and _has_collected(scoped_history, statement.id):
                return self._advance_from_collection_controller(
                    statement,
                    observation,
                    history,
                    evidence=f"collection stop condition satisfied: {frame.stop_reason}",
                    final_read=_ctx(statement, read_inst, frame.collection_scope),
                )
            if frame.boundary_reached and _last_scroll_was_for(scoped_history, statement.id):
                return self._advance_from_collection_controller(
                    statement,
                    observation,
                    history,
        ...
        if statement.scroll_budget > 0:
            budget = statement.scroll_budget
        elif statement.observable_boundary:
            budget = 10
        else:
            budget = MAX_SCROLL_PER_STATEMENT
        if scroll_count > budget:
            print(f"  [Loop] 滚动预算耗尽（{scroll_count}/{budget}，observable={statement.observable_boundary}）→ 结束收集")
            if not _has_successful_scroll_for(scoped_history, statement.id):
                progress = ProgressAssessment(
                    status="exhausted",
                    reason="滚动预算耗尽，但尚未观测到任何成功执行的纵向滚动",
                    source_type="runtime.collection_progress",
                )
                read_inst = None if self.task_type == "action" else _default_read_instruction(statement)
                return self._handle_stuck(
                    statement, progress, read_inst, observation, scoped_history,
                )
            return self._advance_from_collection_controller(
        ...
        last_read_added = bool(
            scoped_history
            and scoped_history[-1].supervisor.statement_id == statement.id
        ...
        if sim_stuck:
            if sim_stuck.frozen:
                if not _has_successful_scroll_for(scoped_history, statement.id):
                    ...
                    read_inst = None if self.task_type == "action" else _default_read_instruction(statement)
                    return self._handle_stuck(
                        statement, sim_stuck, read_inst, observation, scoped_history,
                    )
        ...
        with _Timer(self._timings, self._timings_order, "loop_check", self._token_usage):
            frame = self._loop_check(statement, observation, scoped_history)
        self._last_check = None  # loop statements use _LoopFrameResult, not _SingleCheckResult
        ...
        if frame.loading and not _has_successful_scroll_for(scoped_history, statement.id):
        ...
        if self.task_type == "action":
            ...
            read_inst = frame.read_instruction or _default_read_instruction(statement)

        if frame.should_stop:
            if _has_collected(scoped_history, statement.id):
                print("  [Loop] 已触发停止条件且有采集内容 → 结束收集")
                final_read = _ctx(statement, read_inst, frame.collection_scope)
                if statement.scroll_stop_condition:
                    final_read["collection_summary"] = (
                        f"停止条件「{statement.scroll_stop_condition}」已触发"
                        f"（{frame.stop_reason}）"
                    )
                return self._advance_from_collection_controller(
                    statement, observation, history,
        ...
        if _last_scroll_was_for(scoped_history, statement.id):
            return SupervisorStep(
                should_act=True,
                instruction="继续滚动",
        ...
        with _Timer(self._timings, self._timings_order, "loop_scroll", self._token_usage):
            plan = self._invoke_loop_scroll(statement, frame, observation)
    ...
    def _advance(
        ...
        *,
        ...
        executed_in_scope = any(
            ...
            if t.supervisor.statement_id == statement.id
        )
    ...
    def _handle_stuck(
        ...
        self._monitor.clear_screenshots()
        skip_retry = False
        if history and history[-1].supervisor and history[-1].supervisor.statement_id == statement.id:
        ...
        if not skip_retry:
            self._rt.retry_count += 1

        self._record_failure_constraint(statement, check, history)

        # Early Feasibility probe: a statement already stuck EARLY_FEASIBILITY_AT times is worth an
        # infeasibility check now rather than after it burns every retry. Once per statement; the
        # judge is conservative-toward-feasible, so a "feasible" verdict just continues.
        if (
            EARLY_FEASIBILITY_AT <= self._rt.retry_count < MAX_RETRIES
            and not self._early_feasibility_probed
        ):
            self._early_feasibility_probed = True
            kick = self._maybe_kickback(statement, observation, read_inst, history)
            if kick:
                return kick

        if self._rt.retry_count >= MAX_RETRIES:
            # Feasibility Guard: before giving up, judge if the statement is INFEASIBLE (required control
            # absent) and, if so, kick back to the orchestrator with a re-plan directive.
            kick = self._maybe_kickback(statement, observation, read_inst, history)
            if kick:
                return kick
            return self._fail(statement, check, read_inst)

        print(f"  [Replan] 第 {self._rt.retry_count} 次重试...")
        ...
        ) if statement.is_iterative else ""
        with _Timer(self._timings, self._timings_order, "replanner", self._token_usage):
            replan = self._invoke_replanner(statement, check, observation, history, extra=iter_extra)
        ...
        if replan.strategy == "force_complete":
            ...
            self._last_check = check
            return self._plan_single(statement, check, observation, history)

        if replan.strategy == "escalate_human":
            # The replanner can give up EARLY (before MAX_RETRIES) by escalating — another give-up
            # path. Give the Feasibility Guard a chance here too: if the statement is infeasible (required
            ...
            return SupervisorStep(
                ...
                **_ctx(statement, read_inst),
            )

        atomic_role, action_family = action_metadata(replan, statement)
        mutation_required = bool(statement.kind == "action" and statement.target_values)
        ...
        return SupervisorStep(
            ...
            summary=f"子目标「{statement.name}」尚未达成，第 {self._rt.retry_count} 次调整策略。{replan.diagnosis}",
            atomic_role=atomic_role,
            action_family=action_family,
    ...
    def _maybe_kickback(
        self, statement: StatementContract, observation: Observation, read_inst: Optional[str],
        history: Optional[list[PolicyTurn]] = None,
    ) -> Optional[SupervisorStep]:
        """Feasibility Guard (goal level): at give-up time, judge whether the statement is INFEASIBLE —
        ...
        # Don't kick back a statement whose terminal submit/save already dispatched successfully:
        ...
        # directly contradict authoritative observation. This does not mark the statement done;
        # it only leaves the feasible-but-stuck route with the local replanner.
        if statement.kind == "navigation":
            target_hints = list(statement.target_controls)
            if history:
                for turn in reversed(history):
                    step = turn.supervisor
                    if step is None or step.statement_id != statement.id:
                        continue
        ...
        if controls_meta.get("coverage") == "partial":
            ...
            return None
        goal = f"{statement.name} —— {statement.success_condition}"
        try:
        ...
        if verdict.feasible:
            ...
            return None
        print(f"  [Feasibility] statement 不可行 → 踢回编排器重规划。{verdict.reason}")
    ...
    def _fail(self, statement: StatementContract, check: _SingleCheckResult, read_inst: Optional[str]) -> SupervisorStep:
        print(f"  子目标「{statement.name}」失败")
        reason = f"子目标「{statement.name}」重试 {MAX_RETRIES} 次后失败"
        return SupervisorStep(
            should_act=False,
            outcome=StatementOutcome.failed(reason),
    ...
    def _record_failure_constraint(
        ...
        last_action = next(
            (t for t in reversed(history)
             if t.supervisor and t.supervisor.statement_id == statement.id
