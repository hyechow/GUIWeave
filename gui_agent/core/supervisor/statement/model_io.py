...
def run_checker(
    ...
    *,
    ...
    """Run the single-step statement checker. Used by both production and evals.
    ...
    if prompts is None:
        prompts = StatementPrompts.neutral()
    if constraints is None:
        constraints = []
    kind_section = prompts.check_kind_sections.get(statement.kind, prompts.check_section_default)
    # 连续调值类（picker 收敛）在 kind 段之上叠加专用段：当前值以滚轮中心带为准、强制输出
    # 当前值/目标值。这是连续操作进展传感器的基础——避免把已推进的拖动误读为"没动"。
    if statement.is_converge:
    ...
    accept_items = split_acceptance_items(statement.success_condition, statement.name)
    msgs = assemble_messages(
        prompt, observation,
        system_blocks=[
            app_identity_block(app_name),
            statement_block(statement, task_type=task_type),
    ...
    def _strip_progress_evidence(r: _SingleCheckResult) -> None:
        ...
        if statement.is_converge and r.status == "done" and r.missing_evidence:
            r.missing_evidence = [
    ...
    def _retry_worthy(r: _SingleCheckResult) -> bool:
        ...
        if statement.is_converge:
            return False
        return statement.kind != "navigation" and not r.visible_evidence
...
_SELECTOR_PROMPT = load_prompt_text("task.statement.knowledge_selector")
...
def run_selector(
    ...
    *,
    ...
    (statement, page_identity), so it only fires on page/statement changes."""
                    ...
                    "parts": [
                        {
                            ...
                            "source": "task.statement.knowledge_selector",
                            "type": "text",
                            "text": prompt,
                            "chars": len(prompt),
                        },
...
def run_planner(
    ...
    *,
    ...
    if constraints is None:
        constraints = []
    _retry = int(getattr(statement, "retry_count", 0) or 0)
    if _retry > 0 and not extra:
        ...
        # Collect dead-end paths from ALL statements (replan diagnoses)
        dead_ends: list[str] = []
        for t in history:
    ...
    return invoke_structured(
        ...
        assemble_messages(
            ...
            system_blocks=[
                statement_block(
                    statement,
                    scroll_stop_condition=statement.scroll_stop_condition or "滚动至列表物理底部时停止",
                ),
