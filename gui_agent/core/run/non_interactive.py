...
Reads are pure-vision: ``structured_read`` off the screenshot only — there is no
host-side URL fetching. A ``navigation`` run always goes through the
supervisor/action-policy loop (visual URL entry, tap, …); there is no programmatic
``navigate(url)`` shortcut, so a navigation never enters this drain.
"""
...
def drive_pending_non_ui(
    *,
    ...
    def _hud(msg: str) -> None:
        ...
        # never pushes a HUD status for them. Push one here so the HUD ticks per non-UI step. (max_turns
    ...
    while cur_run is not None:
        if not (cur_run.kind in {"read", "data_query"} and not is_list_read(cur_run)):
            break
