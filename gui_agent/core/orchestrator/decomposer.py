...
7. **前置状态用「终态」建模，别用只在未完成态才出现的中间界面（尤其登录/认证）**：登录、进入某模式等前置，初始往往已满足。**别建模成「打开登录页 → 输账号密码」**——会话常已登录，登录页根本回不去，该步 success_condition「看到账号密码框」永远不成立、卡死。建模成**一步**「确保已登录」，run_kind=navigation，success_condition 写【认证后的可见特征】（如顶栏用户名/首页主内容/导航菜单），已登录则第一帧就判 done 直接跳过。通则:任何「确保处于某状态」的前置，success_condition 写**目标终态本身**，不写「只有还没做时才出现的那个界面」。
