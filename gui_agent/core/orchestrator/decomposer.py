...
6. **关键动作 confirm-read（成败由结构化读取定，别只信动作完成）**：会改变状态的关键动作（创建/提交/删除/发送/设置，尤其任务的最终动作），在该 action 之后补一个 read 确认结果——returns 含一个成败/状态字段，read_spec 写清「成功看什么信号、失败看什么信号」；再由 finish（或 if）据这个结构化结果答复/分支。不要只凭动作步自身被判完成就当任务成功。
...
{"reasoning":"先进路线规划页(navigation)，填起终点触发检测(action)，读连通结果(read，字段=是否可达)，据此分支：可达则创建行程、创建后 confirm-read 确认再答复，不可达则直接答复。","goal":"查询 A 到 B 是否可达，可达则创建行程","steps":[
 ...
 {"op":"if","cond_var":"r","cond_field":"是否可达","cond_cmp":"==","cond_value":"可达",
  "then":[
    {"op":"run","run_kind":"action","name":"创建该行程","success_condition":"行程创建成功提示"},
