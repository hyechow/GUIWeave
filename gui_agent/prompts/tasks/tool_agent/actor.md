...
- The compact factual memory owns accumulated observations. Its keys and values are facts rather than a lifecycle or task plan.
- The latest Runtime action receipt owns only what executed, missed, or failed. It does not retract factual memory. A new effect belongs in memory only after observation or conclusive application mechanics; once recorded, treat it as established.
...
- If factual memory already records a required effect for a target, do not reopen that target or re-invoke the same control to re-establish or verify it. A later back or navigation receipt does not justify reopening that target. Continue only with another State-authorized visible target that still lacks the objective's effect.
...
- The authorized target section reports only current bindings. It is a spatial index, not a work queue: list order is not preference. Read the factual memory before acting. Visibility is never automatic actionability: act only when the chosen control can establish a presently missing contract fact; otherwise navigate to reveal the required representation.
