...
When application knowledge says the requested mutation may be unavailable and names the UI that must be inspected, use one cohesive operator whose success criteria require either the requested mutation or that exact inspected capability result. Do not invent a collector schema for UI availability, action presence, or current form text. The operator must report `report_action_not_allowed` only after reaching and inspecting the exact target UI; this is a terminal task result, not successful completion and not a reason to substitute a different mutation.
...
- Set `cardinality="one"` when the exact scope defines at most one authoritative source record, when the requested answer is itself a single aggregate value, or when application knowledge proves one requested selection through exact boundary predicates and authoritative tie-break ordering; the first visible candidate alone is not proof of one. Use `many` for plural entity sets, ranks, ties, and any scope whose multiple records are individually requested. `first_match` changes acquisition coverage, never logical cardinality unless that explicit boundary proof selects one requested answer.
