...
- `ctx.gui_worker(*, worker_id, profile=None, goal, success_criteria, data_requirements, actions, max_steps=8) -> WorkerOutcome`
...
- Branch on `outcome["phase"]`. For an anticipated alternative, execute it in Python. When the task-level direction remains valid but a GUI subgoal needs another attempt, call `ctx.replan(...)`. The Runtime reruns this same reviewed program, reuses completed GUI work, retries the failed Worker, and preserves its bounded action experience; it never asks for a new orchestration program. Call `ctx.fail(...)` only for a concrete irrecoverable or unsafe condition.
- End every reachable path with `ctx.finish`, `ctx.replan`, or `ctx.fail`.
- On local replanning, repeat the exact Worker call or use `ctx.worker_result(worker_id)`. Never change a stable Worker specification under the same ID and never redo completed GUI work.

GUI Worker specification rules:

- `success_criteria` must be externally checkable conditions for the complete subgoal.
- `success_criteria`, `data_requirements`, `actions`, and `max_steps` must be inline literal values in the `ctx.gui_worker` call so they can be reviewed before execution.
...
- UI filter-entry formats and collected row formats are independent. Values such as `MM/DD/YYYY` in `filters` describe the UI scope only; never infer a transform's input encoding from them. Transform only the canonical values promised by `field_types`.
...
- Supported GUI action capabilities are `tap`, `type`, `scroll`, and `select_option`.
