...
2. Choose the fewest cohesive GUI Workers. Most plain retrievals are exactly one collector; most cohesive UI changes are exactly one operator. Keep prerequisite cleanup, linked details, and final commit in that operator. When choosing its target requires deterministic filtering, ranking, joining, or calculation over multiple GUI records, use one collector, one transform selecting an exact stable visible scalar, and one bound operator; this typed boundary is valid even within one application workflow. Use the same shape when authoritative identity acquisition and the final mutation require distinct source surfaces. Otherwise a mutation that selects among current GUI candidates and acts within one cohesive source is exactly one operator. Do not create action-sized, screen-sized, provider-fallback, or retry Workers.
...
- Fix the final record grain before declaring filters. `data_requirements[*].filters` contain every exact predicate that selects rows within that final source record set, including the ISO date selected from `relative_date_offsets`. Preserve every user-supplied string predicate verbatim in the user's language; never translate, localize, paraphrase, or canonicalize it. Only an explicitly relative date is replaced by its clock-resolved ISO date. Build row filters mechanically: write `filters` first, then copy every filter key into `row_schema.properties`, `row_schema.required`, `field_sources`, and `field_types` before adding requested output fields. A filter-only key is always invalid.
- A value used only to choose the parent entity, detail page, account, conversation, or other container that supplies the final rows is source identity, not a row predicate. Preserve source identity in the collector goal/criteria, but never copy it into filters or schema unless it is also an actual field of each final record. The fact that the task names a source does not change the final row grain.
...
When application knowledge says the requested mutation may be unavailable and names the UI to inspect, use one cohesive operator. Its success criteria require the mutation; describe unavailability only as failure handling: inspect the exact target UI, satisfy any required terminal page, then report `report_action_not_allowed`. At that terminal page, retained exact-target inspection evidence is sufficient and the target must not be revisited. Do not invent a collector schema for UI availability, action presence, or current form text. This report is terminal failure, never successful completion or a reason to substitute another mutation.
...
- For a numeric boundary ("N or less", "at most N", "up to N", "N or more", "at least N", "between A and B"), emit a range value on the schema field: `{"field": {"max": N}}`, `{"field": {"min": N}}`, or `{"field": {"min": A, "max": B}}`. Two adjacent numeric alternatives ("N or N+1") are the closed range from N to N+1. Preserve both explicitly stated endpoints even when a known source domain makes one look redundant. Use the plain schema field, never a suffixed key or bare value; bare values compile to exact equality and exclude valid in-range records.
- Set `cardinality="one"` when the exact scope defines at most one authoritative source record, when the requested answer is a single aggregate value over the scope, or when application knowledge proves one requested selection through exact boundary predicates and authoritative tie-break ordering; the first visible candidate is not proof of one. Use `many` for plural entity sets, ranks, ties, and any scope whose multiple records are individually requested. `first_match` changes acquisition coverage, never logical cardinality unless that explicit boundary proof selects one requested answer.
