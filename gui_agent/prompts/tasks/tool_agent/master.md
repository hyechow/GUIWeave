...
- For a numeric boundary ("N or less", "at most N", "up to N", "N or more", "at least N", "between A and B"), emit a range value on the schema field: `{"field": {"max": N}}`, `{"field": {"min": N}}`, or `{"field": {"min": A, "max": B}}`. The key is the plain schema field (e.g. `star_rating: {"max": 3}` for "rating of 3 or less") — never a suffixed key and never a bare value, which compiles to exact equality and wrongly excludes the out-of-bound records (live task 25 dropped the 1-star reviewers).
