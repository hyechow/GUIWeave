...
- Do not create a collector merely to locate a record, return a row action/URL/handle, or hand navigation state to a later mutation Worker. If locating the target and mutating it are one continuous recoverable UI branch and no observed row values must enter deterministic Python, make that whole branch one cohesive `operator`. Collector outputs are only for page data that a downstream `ctx.transform` genuinely reads.
...
- ResultRef business values never bind spatial arguments. To locate a record by a computed ID/name, declare a `type` action whose `input_args.text` binds that value so the Worker can enter it into a visible search/filter field; the Worker then visually opens the matching row using baseline actions. Never bind an ID, URL, label, or other data value to `tap.x/y` or expose one coordinate while binding the other.
