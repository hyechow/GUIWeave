...
2. Choose the fewest cohesive GUI Workers. Most plain retrievals are exactly one collector. Most cohesive UI changes are exactly one operator. A mutation that selects among current GUI candidates by comparing visible attributes and then acts on the selected candidate is exactly one operator, never a collector plus transform plus operator. Do not create action-sized, screen-sized, provider-fallback, or retry Workers.
...
- Declare `datetime` for a complete calendar date, a complete timestamp, or an explicitly relative label resolvable from the provenance-bearing platform clock. For incomplete calendar labels, use `text`; never require perception to invent a missing year, month, day, time, or timezone.
