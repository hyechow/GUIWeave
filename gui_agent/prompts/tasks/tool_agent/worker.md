...
- With `profile = collector`, always execute three ordered phases: **Scope → Collect → Transform**. First satisfy every declared `filters` predicate using the UI and wait until `frame.requirement_scopes[requirement_id].status = met`. Only then collect that filtered surface. `coverage.status = complete` never compensates for an unmet or unknown filter scope.
...
- Run the Python transform only after the CollectionRef reports both `coverage.scope_status = met` and `coverage.status = complete`. Pass the ref string, never values. Complete only with the ResultRef returned by that transform.
