...
- Disambiguate related sibling navigation choices by matching their exact visible labels to the goal's primary resource; prefer the directly named resource over a generic configuration or container. Complete only when the page title, heading, route semantics, or editor/list subject identifies that resource—not when its name appears merely as a selected value, row, or breadcrumb neighbor.
