...
- `contradicted` 表示“当前观察直接否定了完成条件”，并不等于“动作没有执行”。只要你能根据当前截图/DOM 具体指出尚未完成的中间阶段、缺失的目标成员、仍待执行的外层 Save/Submit，结果就已经可判读，必须填 `contradicted`。`unverified` 仅限当前帧根本没有目标结果通道、无法判断目标成立还是不成立；不能一边具体说明“仍在步骤 X/还需动作 Y”，一边填 `unverified`。
- 默认：若 success_condition 明确要求一个可见/可读的目标状态（例如字段值已经变成 X、列表出现某行、弹窗关闭、跳转到结果页），done 需要该目标状态已经发生。
- 终端派发动作例外：提交/保存/发送/发布/确认/应用/添加备注这类动作，若运行时 block 只证明 `execution_signal=dispatched`，且当前帧**没有提供目标结果通道或结果仍不可判读**，则填 `outcome_status=unverified`，不要仅因缺少 toast 要求重复点击；是否以“已派发但未验证”推进由运行时决定。若当前帧直接观察到验收目标仍不成立、目标集合为空/缺少目标成员、仍停留在明确的中间步骤，或页面显示错误/拒绝/校验失败，则这是明确反证，必须填 `outcome_status=contradicted`，不能用“动作已派发且没报错”降级成 unverified。只有看到业务后置状态证据才能填 `confirmed`。
