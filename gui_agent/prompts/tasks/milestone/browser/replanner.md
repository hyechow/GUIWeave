...
- ⚠️ **禁止把输入框诊断为「输入不完整 / 被截断 / 仅识别了部分关键词 / 缺少前缀 / 需删除重输」——除非「浏览器 DOM 表单控件」里该控件的 `current=` 确实不等于目标值**。窄输入框（宽度仅几十像素）在截图里只显示滚动后的尾部（已输入 `Olivia zip jacket`，截图里只看得到 `…zip jacket`），这是视觉滚动、不是内容缺失。若 `current=` 已等于目标值 → 输入已正确完成，应转向**下一步**（点 Search/Apply 提交筛选、或读取结果），绝不重输同一个值。
- ⚠️ 子目标是搜索/筛选/查找某个具体目标（产品/记录/条目），而当前页面显示 **0 条结果 / 未找到记录**（`0 records found`、`We couldn't find any records.`、空列表）→ 诊断为**搜索/筛选词太具体，或与系统中实际名称不完全一致**；instruction 应**放宽搜索词重试**：去掉修饰词、改用更短或部分关键词（完整名→核心词，如 'Olivia zip jacket' → 'Olivia' 或 'jacket'），在同一筛选/搜索框重搜。**禁止**因为 0 条就去新建条目（New/创建/添加）、或离开当前筛选页另寻他处——0 条是「没匹配到该词」，不是「该记录不存在、需要创建」。
