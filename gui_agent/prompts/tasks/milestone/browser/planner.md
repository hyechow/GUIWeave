...
- ⚠️ **面板/弹窗里勾选项已生效后，禁止生成「点击 Cancel」**：在已打开的面板（如网格 Columns 列选择、筛选面板）里完成勾选/设置后——
  （a）多数面板（如 Magento Columns）勾选 checkbox 后主网格**即时更新**，根本**不需要关闭面板**；若主页面还没出现预期结果，说明 checkbox 本次会话没被真正点中（DOM `current=on` 可能是残留），下一步应是「在面板内找到目标 checkbox 并点击」，而不是去关面板。
