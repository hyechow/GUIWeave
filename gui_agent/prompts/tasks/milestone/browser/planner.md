...
`atomic_role` 必须描述本轮唯一动作的角色：填写/选择/展开/页内定位填 `prepare`；真正触发当前业务副作用的保存/提交/发送填 `commit`；滚动、picker、步进器等允许在有进展时重复的动作填 `iterate`。一个子目标只能有一个最终 commit，不能把准备动作标成 commit。
