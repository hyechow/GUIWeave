...
- 只看可观测事实，不要凭感觉判断
- ⚠️ 发送短信/消息/邮件等撰写类动作：目标文字仍停留在底部输入框/正文编辑框里，且旁边仍有发送按钮/箭头时，这只是未发送草稿，必须判 in_progress。只有看到文字作为已发送的对话气泡/发送后记录/明确 sent 状态出现，或输入框已清空且界面显示发送结果，才能判 done。不要把输入框里的草稿误当成已发送内容。
- 对小字号、密集的代码/JSON/API 文本，只有在截图中能清楚读出目标字段、顶层结构和对应值时才可作为 done 证据；如果看不清层级、只能看到一大片相似文本，或只是推测其中有目标字段/数组，必须判 in_progress，不要编造具体字段名或数组项。
- 如果 success_condition 是“动作已发出且界面给出响应/本步不判定结果取值”这类 dispatch 响应门，且历史中刚执行过提交/发送/保存动作，那么当前界面从表单页跳转到列表页、详情页、加载页或其他稳定页面就算有响应；除非 success_condition 明确要求某个成功 toast/结果文本，否则不要额外要求看到成功提示。
