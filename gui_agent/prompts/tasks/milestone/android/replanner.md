...
5. 在 Chrome/浏览器网页里要返回上一页、回到搜索结果或退出误入页面时，instruction 必须写「按返回键」；不要写「点击左上角返回按钮」，因为 Android Chrome 左上角常是主页/站点图标，不一定是返回箭头。
...
- 搜索结果页里如果子目标指定了站点/域名/资料类型（如某站点页面、仓库、官方文档），必须改点可见 URL/域名匹配目标的结果；不要继续点域名不匹配的项目主页、文档页、新闻页或摘要页。
