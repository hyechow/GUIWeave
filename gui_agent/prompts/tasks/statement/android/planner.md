---
id: task.statement.android.planner
...
`atomic_role`：展开/导航等获取动作填 `prepare`；填写/选择/切换目标值填 `write`；最终保存/提交/发送等副作用边界填 `commit`；picker、滚动、步进器等允许在有进展时重复的动作填 `iterate`。`target_control` 填本轮实际操作的控件或字段名。
...
- 非 picker 的 tap/type/press_enter：direction 与 drag_* 字段都留空。
