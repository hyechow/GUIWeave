...
上下文，也禁止另造 `rows`、`records` 等未声明变量。Data 不能消费这些集合。
...
输出仍严格使用 `Interact / Acquire / Data / Compute / Command / If / ForEach / Finish`。不要把纠正内容翻译成页面步骤、
...
Interact 只纠正 UI 后置条件，不声明业务 bind/returns。若需要读取纠正后的当前页面事实，紧接 Data read；
可读且不满足由 Program If 再进入纠正 Interact，不可读则由 Data 返回带证据的 kickback。若需要继续对
已完成集合做筛选、分组、聚合、排序或投影，使用一个 Compute，不要让 Data 重复处理集合。

若 Data kickback 证明当前 observation 缺少事实，不要重复同一失败 source；为剩余 Data 安排新的
