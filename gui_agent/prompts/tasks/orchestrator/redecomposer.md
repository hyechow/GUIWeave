...
env 中；需要继续计算时必须按给出的原变量名通过 `inputs` 引用，禁止要求把记录正文复制进上下文，
也禁止另造 `rows`、`records` 等未声明变量。
...
同一直线 block 禁止连续 Data。已有集合需要继续处理时，通过一个 Data 的 inputs 引用它，并在该 Data
内部完成全部线性变换、直接返回剩余 Program 消费的最终结果；不要输出中间工作表后再接第二个 Data。

Interact 只纠正 UI 后置条件，不声明业务 bind/returns。若需要判断纠正后的业务数据是否满足，紧接 Data
重读；可读且不满足由 Program If 再进入纠正 Interact，不可读则由 Data 返回带证据的 kickback。
