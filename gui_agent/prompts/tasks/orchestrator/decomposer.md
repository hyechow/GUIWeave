...
- op="run"：声明一个命令或查询 primitive。
    ...
    · target_values：action 若能从目标和应用功能说明得到明确字段终态，必须填写 `{语义字段名: 目标值}`。重复集合的一行需要多个字段共同有效时，必须把同一行的所有必需业务字段都列出；执行层据此区分 complete / incomplete / unknown，并阻止“只填了邻接字段就直接保存”。filter 必须声明完成后的**完整筛选状态**，包括本步要保留的上游筛选；不要靠“保留/追加/清除残留”等散文让运行时猜。没有明确字段终态时留空，不得猜测 DOM 字段名。
