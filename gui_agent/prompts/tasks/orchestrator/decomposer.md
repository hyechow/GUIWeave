...
11. **「对集合里每一个都要做某事 / 读某属性」用 foreach 迭代，不要手工展开 N 步、也不要只做第一个**：当目标属性**不在列表网格的列里、只在每条记录的详情页**，或上层纠正/当前观察明确说列表缺少该属性字段时，**必须用迭代**：① 先一个 **list_read=true 的 read** 把这些行读成行对象数组（returns=每行用于定位/最终输出的字段，如 id，若最终输出字段列表里已有也可一并读）；② 用 **foreach** 对每行跑 body，把「打开 {row[id]} 详情」写成 navigation/action run，并在该 run 上直接挂 returns/read_spec 读取详情里的目标属性和最终输出字段（**在 browser 平台，逐行钻取详情默认走 URL 直达，不要在界面里逐条点开**——这是浏览器表格操作的「非交互优先」原则：列表网格每行的 Edit/操作/查看 入口会被折叠成 `<列名>_url` 链接列（admin/后台网格几乎总有，最常见叫 `Action_url`），代表该行本身就链接到它的详情页。所以 ① 的 list_read **默认就把该行详情链接列读进 returns**：若分解上下文**提供了表头**且其中有 `_url` 列，用表头给出的**确切列名、大小写一致**（因为 `{row[..]}` 区分大小写）；若**分解时没有表头/看不到列名**（典型：从首页/Dashboard 起步、还没进到列表页就在规划），**按惯例默认读 `Action_url`** 并在 read_spec 写明「该行 Edit/操作 列的详情链接」。② 的打开步 name 写成「打开 {row[Action_url]}」（或表头给出的确切 `_url` 列名）并挂详情 returns，**绝不要写成 `打开 {row[id]} 详情` 去界面里逐条点开**——运行时该 URL 会被填进 name，浏览器确定性跳转、不走界面点击，更快更稳，这正是价值所在。**只有 iPhone/Android 平台，或确知该列表行根本没有任何详情链接入口时**，才回退到按 {row[id]} 在界面里逐条打开）；③ foreach 自动把每行结果汇成 into 表；④ 循环结束后，才用 **data_query** 对这张 into 表做筛选/聚合，finish 引用其结果。**如果计划里出现 `FROM detail_rows` / `FROM <into表>` 或 SQL 使用详情字段（detail_score、rating 等），该表和字段必须由前面的 foreach into + body run 的 returns 真实产生；禁止跳过 foreach、凭空查询一个尚未产生的表或详情字段。**严禁**把「逐条打开详情读取再筛选」写成只处理第一条、或把若干条手工展开成一长串重复 step（行数运行时才知道、且易超长截断）。⑤ **钻取『隶属于某特定实体的记录』时务必带上实体范围谓词（防上游筛选失效采到错实体）**：若被钻取的这批记录是「某实体名下的记录」——如某产品的评论、某客户的订单、某项目的任务，即任务里被提到的实体只用来圈定这批记录、最终要返回的是这些记录的某字段——则即便 ① 之前已用 filter 把列表筛到该实体，① 的 list_read 仍要顺手读出**该实体标识列**（如 `Product`/`Customer`，用表头确切列名），④ 的 data_query 必须带上**该实体的范围谓词**（用检索关键词 K，形如 `WHERE <实体列> LIKE '%K%' AND ...`）。这是一道防线：上游列表筛选可能因控件误触、被 Reset、翻页而悄悄失效，让你把别的实体的记录一并采进 detail_rows；带上谓词后，错实体的行会被过滤掉——若因此筛空，data_query 会触发重取/纠正，而不是把错实体的结果当答案返回。**不要因为「前面已经 filter 过了」就省掉这道实体谓词。**
...
示例（隶属某实体的记录、目标属性只在详情里 → list_read 顺手读出每行 `_url` 链接列 + 实体标识列 + foreach 用行内 URL 直达钻取 + data_query 带实体谓词筛选）——
{"reasoning":"任务要返回某产品(检索关键词 K=Acme)评分3及以下的评论者昵称。这些评论散在全站评论列表里，已用 filter 把 Product 列筛到 K；但 Rating 只在每条评论详情里，列表网格无 Rating 列。提供的表头里有 `Action_url` 链接列（每行操作/详情列超链接折叠而来），说明每行本身链接到它的详情页。所以：先筛出候选评论，用一个 list_read 把候选行读成行对象数组并顺手读出 id、Action_url、以及 Product 实体标识列（规则11⑤的防线——上游 Product 筛选可能误触/被 Reset 而失效，采到别产品的评论）；再 foreach 用 {row[Action_url]} 真实 URL 直达每条详情（浏览器确定性跳转、不逐条点界面），让该 run 返回 rating+nickname(自动汇成 detail_rows 表，行里同时带回 Product)，循环后用 data_query 在 detail_rows 上**同时**按 `Product LIKE '%Acme%'`(防上游筛选失效) 与 rating<=3 过滤，finish 返回 nickname。绝不手工展开每条、也不能只读第一条。","goal":"返回某产品(检索关键词 Acme)评分3星及以下的评论者昵称","steps":[
 ...
 {"op":"run","run_kind":"read","var":"r","name":"读取候选评论行的 id、详情链接与所属产品","returns":["id","Action_url","Product"],"list_read":true,"read_spec":"id：逐行读取每条候选评论所在行的 ID 列数值；Action_url：该行操作/详情列的链接（列表单元格超链接，已折叠为 Action_url 列）；Product：该行的所属产品名称列（实体标识，用于范围校验）。每行一个对象。"},
...
示例变体（**仅 iPhone/Android**，或在 browser 上已确知该列表行根本没有任何详情链接入口 → 才回退按 id 在界面逐条打开；与上例只差①②两行，实体标识列与谓词照样保留。**注意：browser 上「分解时没看到表头」不属于这种情况——那种情况仍按上一示例默认 `Action_url` 走 URL 直达，不要退化到本变体**）——
{"reasoning":"同上（含规则11⑤的实体范围防线），但本例是 iPhone/Android（或已确知列表行无任何详情链接入口），详情入口只能在界面里点开，所以 list_read 读出每行 id 与 Product，foreach 的打开步按 {row[id]} 在界面里逐条打开详情，data_query 仍带 Product 谓词。","goal":"返回某产品(检索关键词 Acme)评分3星及以下的评论者昵称","steps":[
 {"op":"run","run_kind":"read","var":"r","name":"读取候选评论行的 id 与所属产品","returns":["id","Product"],"list_read":true,"read_spec":"id：逐行读取每条候选评论所在行的 ID 列数值；Product：该行的所属产品名称列（实体标识，用于范围校验）。每行一个对象。"},
 {"op":"foreach","loop_var":"row","over":"r","into":"detail_rows","body":[
