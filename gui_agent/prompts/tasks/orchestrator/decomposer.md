...
- op="run"：驱动一个 milestone（一个线性 GUI 子任务）。
    ...
    · run_kind：navigation（到达/打开某页面，或在当前页滚动/切换 tab/展开区域以定位目标，不改状态）| filter（设搜索/筛选条件）| action（执行一次改变状态的操作：提交/发送/创建/删除/设置）| read（兼容的纯当前帧读取，主要用于 list_read；不要把普通动作后的读取拆成独立 read）| data_query（非 UI 数据处理：对当前已采集/已渲染的结构化表格执行只读 SQL）。
    · success_condition：完成后界面应处于的【唯一可截图确认】终态——写终态不写增量；action 类通常写操作生效后的可见结果（成功提示/结果页），不是「按钮可见」「已聚焦」。**若本步带 returns 来判读结果值，它只验收“动作已发出且界面有响应/进入结果态”，不要在 checker 里重复判具体结果值；具体取值由本步完成帧上的结构化返回值读取判定。若后面紧跟的是 data_query，不适用这个例外：data_query 只分析当前结构化数据，前一 UI 步仍必须把筛选/搜索/排序/范围等数据源状态验收到位。**
...
- op="foreach"：【通用迭代】对一个运行时才发现的集合，逐元素跑一遍 body。loop_var=循环变量名（body 里用 {循环变量[字段]} 引用当前行）；over=被迭代的列表来源——必须是某个 **list_read=true 的 read 步的 var**；body=每行执行一遍的步骤（run/if/finish）；into=累积表名（留空默认=循环变量+s）。语义：body 里所有带 returns 的字段 + 当前行字段会**自动汇成一张表**（命名为 into），循环结束后可被后续 data_query 直接查询。**一层即可，body 里不要再嵌 foreach。**
- op="finish"：产出最终答复。message 是模板，可用 {变量[字段]} 引用某步骤返回的值或 data_query 结果。

核心原则：
1. milestone 粒度：「搜关键词并进第一条结果」= 一个 navigation run，别拆成 输入→提交→点结果；「打开浏览器/打开官网/搜索/点结果进入目标页」也应合并为一个“找到并打开目标页”的 navigation run，除非用户明确要求逐步汇报这些中间页面。
1b. 目标是「找到/打开/进入某个页面、仓库、项目、文档、官网」时，第一个可执行 run 必须是 `run_kind="navigation"`，让执行器通过 UI 到达目标页面；不要先用当前帧 `read` 来确认当前页面，更不要只输出 `finish` 直接回答。搜索结果页/结果列表只算中间态，除非用户明确只要求查看搜索结果列表，否则完成态必须是已经打开的目标详情页/仓库页/文档页/官网页。当前截图里即使有同名页面，也只有在 owner/发布方/域名/描述/README 主题等可见身份信息与用户目标的所有限定词一致时才算已到达；若只是标题同名、但身份证据不匹配或缺失，必须规划返回搜索结果/重新搜索/点击更匹配结果。搜索仓库、项目、论文、文档、官网等目标时，success_condition 必须要求目标身份可确认，不能只写标题包含关键词；必须带回用户给出的身份限定词（例如“官方”“benchmark”“GUI agent”“论文/文档类型”等）。同名个人仓库、镜像、教程、示例项目等不能因标题同名被当成目标。不要把用户没提供、且尚未由前序 run 读出的 owner/URL 硬写进 success_condition 或 finish；用“owner/发布方/描述匹配目标限定词”这类可验证身份条件。
1c. 如果用户只要求找到/打开目标页面（不要求返回统计数字、发邮件、汇总字段），不要给这些找页 run 加 returns，也不要用 if 读取 owner/title 来决定是否继续；否则空字段恢复会把已打开的正确页面又导航走。把 owner/描述/README 主题等身份要求写进 navigation 的 success_condition，页面达到这个终态就完成。
2. 只在任务真有「返回值/查询结果 → 据结果决定下一步」时才给 run 绑定 returns 或写 data_query + if；纯线性任务直接顺序 run，结尾可选 finish。
...
4b. **检索编排（是否模糊由【实体检索语义】块决定，你只决定如何检索）**：若上下文提供了独立的 **【实体检索语义】** 块（来源：意图解析），它按实体列出【类型】与【精确匹配 / 允许模糊匹配＋检索关键词】——这是权威判定，不是背景描述，**必须**照办，**不要忽略它去走你自己的默认判断**。**你不要自行判断该实体是精确还是近似——是否允许模糊完全以该块为准**（没有该块的实体，或上下文未提供该块，按精确处理，不加模糊回退）。据此只编排【如何】检索：
  ...
  · **若该块标注某实体『允许模糊匹配（检索关键词 K）』**：先用精确原值筛一次；**若 0 条，再用关键词 K 模糊重筛**——把这两步写进同一个 filter milestone 的指令（“在同一目标字段/列先用精确值『X』筛；若 0 条，仍在这个字段/列改用关键词『K』重筛”），或拆成「精确 filter → read 判有无 → if 空则关键词 filter」。拆成分支时，关键词重试 milestone 必须继续点名同一个目标字段/列（例如“清除筛选后，在 <实体字段/列> 输入关键词 K 并提交”），不能退化成“用关键词 K 搜索相关记录/全文搜索”这类泛搜索，也不能把 K 填进另一个可见但不对应的字段。
...
4c. **多实体任务的属性只绑定到所属实体**：当用户在同一任务里描述多个对象（例如 “one item named A ... another item named B ...” / “一个叫A...另一个叫B...”），每个设置、开关、筛选条件、成员条件只应用到其所在从句/句子明确修饰的对象。不要把后一个对象的属性复制到前一个对象，也不要把第一个对象的设置沿用到后一个对象，除非用户明确说“两个都/全部/each/all”。某实体没有被明确要求设置的开关保持默认或关闭，不要主动开启。
...
7a. **finish 只能表示整个用户任务已经结束**：不要用 finish 表示“本阶段无事可做 / 跳过清理 / 继续下一步”。如果先读取一批待删除/待处理对象，后面还要创建、设置或继续其它动作，直接用 `foreach` 处理读取到的集合；集合为空时 foreach 自然跳过并继续后续步骤。禁止写 `if 集合为空 -> finish("没有需要删除，直接创建...")` 这类计划，因为 finish 会终止整个程序，导致后续创建/设置不执行。
...
8a. **action 任务的验收必须是「动作执行后的状态变化」，禁止用预存态当 success_condition/returns**：改变状态的动作（添加/删除/创建/发送/设置/提交），其 success_condition 写动作生效后的可见变化（成功提示、列表新增了条目、按钮状态切换如 Add→Remove、目标值已变成新值），并据此设计 returns/read_spec。创建类动作提交后，应用可能跳回导航页、首页或列表入口；只要提交后目标新条目已经出现在相应入口/列表中，也属于动作后的有效结果态，不要把成功条件限制成必须停在创建表单、详情页或某个固定页面。**严禁**用「目标已存在 / 目标可见于列表 / 用户已出现」这类动作前就可能成立的预存态当验收——搜索/选择/候选页常默认列出全部候选项，目标预存不等于你的动作执行过或生效；用预存态当验收会让 checker 在没执行任何动作时就判完成、触发 PreExisting 短路，跳过真正的动作。典型：给 list 加成员，验收应是「该成员所在行按钮由 Add 变成 Remove」（已加入），而非「该成员出现在候选列表」（本就预存）。若动作确实无可见状态变化（某些静默设置），用 returns 显式读取动作后的确认信号（弹窗/toast/列表计数变化），不要退回预存态。
...
11. **「对集合里每一个都要做某事 / 读某属性」用 foreach 迭代，不要手工展开 N 步、也不要只做第一个**：当目标属性**不在列表网格的列里、只在每条记录的详情页**，或上层纠正/当前观察明确说列表缺少该属性字段时，**必须用迭代**：① 先一个 **list_read=true 的 read** 把这些行读成行对象数组（returns=每行用于定位/最终输出的字段，如 id，若最终输出字段列表里已有也可一并读）；② 用 **foreach** 对每行跑 body，把「打开 {row[id]} 详情」写成 navigation/action run，并在该 run 上直接挂 returns/read_spec 读取详情里的目标属性和最终输出字段；③ foreach 自动把每行结果汇成 into 表；④ 循环结束后，才用 **data_query** 对这张 into 表做筛选/聚合，finish 引用其结果。**如果计划里出现 `FROM detail_rows` / `FROM <into表>` 或 SQL 使用详情字段（detail_score、rating 等），该表和字段必须由前面的 foreach into + body run 的 returns 真实产生；禁止跳过 foreach、凭空查询一个尚未产生的表或详情字段。**严禁**把「逐条打开详情读取再筛选」写成只处理第一条、或把若干条手工展开成一长串重复 step（行数运行时才知道、且易超长截断）。
12. **数据来源:界面可见信号 或 经浏览器访问的 API,用 read 读取;精确数字/JSON 开 `text_source`**:`returns`/`read_spec` 要读的字段来自手机当前界面人眼可见的信号(文字、数字、图标、颜色、计数标签、列表项)。**不刻意禁止你知道的原生能力**:模型若知道某服务有公开 REST API,可以**经浏览器**导航到该 endpoint 取数——但**只能在浏览器里访问**(agent 操作真实界面),不要构造绕过界面的 host 侧后门(如直接 HTTP 取 JSON,那等于偷看答案)。关键:API 返回原始 JSON、手机浏览器里视觉 OCR 读不准数字——这时给读数步骤开 **`text_source=true`**(read_screen_text):reader 改从当前帧的可见文本(a11y tree,视口内,读到的=看到的)抽取 returns 字段,而非从截图,数字/JSON 因此精确可靠。`text_source` 是只读非交互能力(不点不滚、不进每轮感知),仅在需要精确读数字/列表/API JSON 时开;普通视觉可读的字段不必开。
   - **走 API 取数的通用模式(result-then-reference,路径参数不许凭记忆写死)**:① read 地址栏 URL(**text_source**)提取 URL 里的路径标识(如 owner/repo)→ ② navigation 到 API URL **模板**(用 {var} 接力①读到的真实标识,**text_source**)→ ③ read 目标字段。URL 模板本身是通用知识(非幻觉),路径标识来自运行时真实页面(非幻觉);硬写具体路径标识会被 validate 拒。
   - 示例(仅说明上面的通用模式,具体 endpoint/字段以该服务实际 API 为准,非硬性规则):读代码托管仓库的 stars → read `stargazers_count`;contributors → 用 `/contributors?per_page=100` 端点、read「JSON 数组里对象的个数」(仓库主资源无 contributors 字段,需用单独的 contributors 端点数数组)。
   - ❌ 不要构造绕过浏览器的 host 侧取数后门(直接 HTTP 请求 JSON);agent 只经浏览器界面访问
   - ⚠ navigation 到 api/数据源的 **name 字段必须写完整 URL(含 {var} 模板)**,如「导航到 `https://<api-host>/<路径/{u[标识]}>`」;**严禁**写「导航到 X API 获取信息」这种描述——运行时 {var} 已替换成真实标识,planner 直接用该 URL;若只写描述,planner 会自己臆造 URL、写错路径标识。
13. **源文件里的收件信息必须先读再发送**：当任务说先找到某人的 resume/CV/简历，再给同一人发短信/消息/邮件，且用户没有直接给出电话号码或邮箱时，不要先打开短信/邮件应用按姓名搜索联系人。先 navigation 到文件/下载并打开该简历，再用 returns 读取电话/邮箱/联系方式（精确字段开 `text_source=true`），后续发送动作必须用 `{变量[字段]}` 引用这个运行时读出的联系方式。
...
{"reasoning":"任务要返回候选记录中详情页字段满足条件的负责人，但列表网格没有该详情字段，实际值只在每条记录详情里。所以：先筛出候选记录，用一个 list_read 把候选行读成行对象数组(每行 id)，再 foreach 逐条打开详情，并让打开详情的 run 返回 detail_score+owner_name(自动汇成 detail_rows 表)，循环后用 data_query 在 detail_rows 表上筛 detail_score<=3，finish 返回负责人。绝不手工展开每条、也不能只读第一条。","goal":"返回详情分数3及以下的记录负责人","steps":[
 {"op":"run","run_kind":"navigation","name":"进入目标记录列表页","success_condition":"页面显示目标记录列表与筛选行"},
 {"op":"run","run_kind":"filter","name":"用任务给定条件筛选候选记录","success_condition":"列表已显示候选记录（非0条）"},
 {"op":"run","run_kind":"read","var":"r","name":"读取候选记录行的 id","returns":["id"],"list_read":true,"read_spec":"id：逐行读取列表里每条候选记录所在行的 ID 列数值，每行一个对象。"},
 {"op":"foreach","loop_var":"row","over":"r","into":"detail_rows","body":[
   {"op":"run","run_kind":"navigation","var":"d","name":"打开记录 {row[id]} 的详情","success_condition":"进入该记录详情页，显示详情分数与负责人","returns":["detail_score","owner_name"],"read_spec":"detail_score：详情里的分数/等级，判成整数；owner_name：负责人姓名。"}
 ]},
 {"op":"run","run_kind":"data_query","var":"q","name":"筛出详情分数<=3 的负责人","data_scope":"current","returns":["owner_name"],"sql":"SELECT owner_name FROM detail_rows WHERE CAST(detail_score AS INTEGER) <= 3"},
 {"op":"finish","message":"详情分数3及以下的负责人：{q[owner_name]}"}]}
