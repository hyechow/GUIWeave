...
- op="run"：声明一个命令或查询 primitive。
    ...
    · **按资源依赖规划 mutation**：写 steps 之前，先从目标、应用功能说明和当前观察中列出本任务要改变的持久化资源、每个资源的所有者类型，以及资源间的前置依赖；再按依赖做拓扑排序，最后才为每个资源选择页面和导航。若目标资源会消费另一个必须先存在/先持久化的资源或选项，整个前置资源阶段（到达→修改→保存）必须排在进入目标资源编辑阶段之前；不能先打开消费者，再离开补依赖后返回。值是否允许成为前置资源由 Router 角色决定：`target_value` 可以，`qualifier_value` 只能在最终 mutation 中选择，禁止为它规划独立的定义 mutation。没有明确知识或观察依据时，不得臆造额外前置资源。
    ...
    · **业务终态与动作反馈分离**：action 的 `target_values` 声明要实现的幂等业务终态；已有权威状态已满足时不得重复写入。没有结构化终态的发送/提交/删除等命令优先按可观察后态验收；没有稳定反馈通道时，运行时只把可靠派发回执降级记为 `accepted_unverified`。不要用成功 toast 是否出现来决定动作有没有执行。
    · **持久化边界显式声明**：action 写入业务字段后若仍需独立 Save/Submit 才持久化，填 `persistence:"explicit_commit"`；动作本身立即生效填 `persistence:"immediate"`。它只描述持久化协议，不填写按钮名称，也不能靠 success_condition 中出现“保存”来隐式推断。
    ...
    · target_controls：navigation/filter/action 必须声明本步要命中的业务字段、控件或集合能力名称。命名列筛选必须填该列名，不能填通用搜索框；mutation 只填被改变的业务字段或集合名，不写 Save/Submit/Apply 等持久化按钮，也不写 CSS、XPath、坐标或具体点击序列。
    · target_values：action 若能从目标和应用功能说明得到明确业务终态，必须填写 `{语义字段名: 目标值或目标值数组}`。单值写字符串；**同一个选择控件/选择组**必须同时选中多个值时写字符串数组（如 `{"Size":"XXS","Color":["blue","purple"]}`），禁止写成 `"blue and purple"`/`"blue 和 purple"`。数组不表示重复集合里的多行；多行成员必须由各自行身份与标量字段描述。意图解析给出的 target_value/qualifier_value/value_members 必须被相关 mutation 消费；qualifier_value 只进入最终选择，不得触发创建或改写其上游定义，只有 target_value 才允许这样的前置。它只描述期望结果，不标识可写对象，也不授予写权限；目标身份与单次写授权由执行层根据运行时观察产生。系统自动生成的 ID、时间戳等派生元数据不用声明。重复集合的一行需要多个字段共同有效时，必须把同一行的所有必需业务字段都列出；执行层据此区分 complete / incomplete / unknown。filter 必须声明完成后的**完整筛选状态**，包括本步要保留的上游筛选；不要靠“保留/追加/清除残留”等散文让运行时猜。没有明确字段终态时留空，不得猜测 DOM 字段名。
    · success_condition：完成后界面应处于的【唯一可截图确认】终态——写终态不写增量；action 类通常写操作生效后的可见结果（成功提示/结果页），不是「按钮可见」「已聚焦」。**若本步带 returns，success_condition 仍须确认交互已经到达可读取结果的稳定完成态；不要只写“动作已发出/界面有响应”，也不要重复判具体返回值，具体取值由完成帧上的结构化返回值读取。若后面紧跟 data_query，前一 UI 步仍必须把筛选/搜索/排序/范围等数据源状态验收到位。** **⚠️ filter（设筛选/搜索）类 step 的 success_condition 只写【筛选控件自身的终态】——可见的已应用筛选标记（active filters）、控件值、结果计数或列表已刷新；这是“筛选是否已生效”的权威信号，由筛选控件状态决定。绝不要在 filter 的 SC 里写“列表显示满足条件的行/产品”这类要求逐行复核行内容的措辞（如「显示库存为 3 的产品」「列表只剩 X 的记录」）：行/单元格里常有与被筛列同名或相邻的展示列，这会让验收误判、把一个已正确生效的筛选反复清除重设而打转。筛对了 = active filters 等于 target_values 声明的完整筛选集，不靠数行。** 搜索框/keyword box 这类 free-text search 的 filter step，name 必须写成“输入/填入 X 并提交搜索”（按 Enter、点击搜索图标/放大镜，或按应用知识指定的搜索按钮）；只写“填入 X”或“搜索 X”不够，运行时可能只改了输入框 current 值而没有应用筛选。
    ...
    · SQL 布尔枚举：foreach/read 的界面返回值按 TEXT 入表；true/false 必须按文本比较（如 `lower(has_value) = 'false'`），不能写 `has_value = False`。
...
- op="foreach"：【通用迭代】对当前页面的一个集合（列表/网格），逐行跑一遍 body。name=该采集操作的描述（如「采集候选评论行」）；loop_var=循环变量名（body/body_goal 里用 {循环变量[字段]} 引用当前行）；`row_fields`=从当前列表/网格每行先采集并绑定到 loop_var 的字段列表（系统自动从当前页面的语义树/网格直取，**无需在 foreach 前另写 read 步**）；into=累积表名（留空默认=循环变量+s）；body=每行执行一遍的步骤（run/if/finish）；`output_fields`=循环结束后 into 表承诺额外产出的字段（主要用于 body_goal 或 body 内 compute/call/run 的对外字段）。旧计划可用 `returns` 兼容：body=[]/显式 body 时 returns≈row_fields；body_goal 且无 body 时 returns≈output_fields。新计划优先写 row_fields/output_fields，不要让 returns 同时承担两种语义。
  ...
  **⚠️ body_goal（逐行子目标，替代写死的 body）**：当每行要做的子任务**太复杂、固定步骤拼不可靠**（典型：需要从行字段**派生**一个键、用它**搜索另一个关联实体**、在结果里**按谓词判别**正确的行、再打开读属性——即 per-row 实体 join），不要硬写 body 的多步 run，改填 `body_goal`：一句**含字面模板 `{loop_var[字段]}`** 的子目标文本（运行时按行代入），它会被**当场重新分解**成子程序、由 agent 自己 plan/replan 完成。必须同时写 `row_fields`（body_goal 需要引用的当前行字段，如 sku/detail_url/current_price）和 `output_fields`（每行子目标最终返回/计算/保存后的字段，如 old_price/new_price/status/size，后续 data_query 只能查询 row_fields + output_fields）。`body_goal` 与 `body` 互斥；**子目标内不得再用 body_goal（只允许一层）**。**成员判定的首选形态 = member_desc + 显式 body**：当「每行做什么」是机械的、知识能确认的步骤（打开行 URL→读值→算→填→存），但「哪些行属于目标集合」依赖运行时数据（尺寸/颜色编码、状态、数值）时，**写显式 body + 把成员归属写进 `member_desc` 字段**（语义描述，如「size 28 的目标变体」）——运行时会在采集到真实行后做一次圈选、只对成员行执行 body。member_desc 同样**只写语义描述,绝不写猜测的字面量谓词**。body 里不要再写成员判定的 if/compute（圈选已在入口完成）。
  member_desc + 显式 body 形状示例（首选渐进式集合处理）：`{"op":"foreach","loop_var":"row","row_fields":["record_id","detail_url"],"into":"updated_rows","member_desc":"属于目标集合的子记录","body":[{"op":"run","run_kind":"navigation","var":"d","name":"打开 {row[detail_url]} 进入记录详情页","success_condition":"已进入该记录详情页，显示目标数值字段","returns":["current_value"],"read_spec":"current_value：读取目标数值字段当前值"},{"op":"compute","var":"new_value","expr":"round(current_value * 0.865, 2)"},{"op":"run","run_kind":"action","name":"确保目标数值为 {new_value} 并保存","success_condition":"持久化后的目标数值为 {new_value}","target_values":{"目标数值":"{new_value}"},"persistence":"explicit_commit"}]}`。
...
- op="call"：调用一个**函数**（见下方 functions）。func=函数名，call_args=参数名→取值模板（`{row[字段]}`/`{标量}`/字面量，在调用处作用域解析），var=接函数返回值（一个 StatementOutcome，后续 `{var[返回字段]}`）。**函数与循环无关**，可在 main / if / foreach body 任意处调用。
...
4c. **cardinality 表示逻辑覆盖范围；先判 mutation 的资源所有者，再决定执行次数**：【实体检索语义】块的 cardinality 是权威。某实体标了 **『多目标(一组)』**（cardinality=set，并给出成员条件，如 `size 28`）时，它是一个规格、覆盖多个逻辑成员，但这**不自动等于 foreach**。`collection_scope` 只是已命名 owner 下的覆盖范围，禁止对该短语构造 exact→fallback 检索；只对 `lookup` 执行检索规则。**每个 collection_scope 必须原样复制到负责它的 foreach.member_desc/body_goal，或聚合 mutation 的 covers_set；采集其他实体的 foreach 不能算覆盖该 scope。**先查应用功能说明：目标字段/关系究竟由每个成员分别拥有，还是由一个父对象/容器/批量资源聚合拥有。
...
8. **关键动作必须有明确验收合同，但不要制造返回值**：会改变状态的关键 action 必须写清持久化后的期望终态；动作执行、动作反馈与业务效果是不同证据，运行时按可用信号验收，steps 不因某种反馈可能缺失就虚构 `save_status`/`has_value`。已声明 `target_values` 的 action 本身就是幂等终态合同：不要先读取 `has_x`/`option_exists`，再用 compute/data_query/if 决定是否执行同一个目标 mutation；目标成员已存在时的跳过与缺失成员补齐由该 interactive Run 在实时页面内处理。顶层 DSL 分支只用于读取结果会改变后续资源或操作类型的情况。只有后续 if / finish 真正需要读取某个结果值，或动作本身是检测/查询且结果决定后续流程时，才在 action 上声明 returns/read_spec；此时 success_condition 只验收动作已发出且界面有响应，具体值由 returns 读取。若结果需要表格聚合，则在数据源准备好后用 data_query。**任务动词是 update/set/write/change/delete/create 等修改类时，steps 必须包含最终 mutation action；修复前序读取/data_query 问题时，不得把写入/保存阶段删掉，只改坏的读/查结构并保留最终写入。**
...
{"reasoning":"目标是某实体范围内最近一笔满足状态条件的记录。先按实体字段检索，若允许模糊则用 f1+if 做精确→关键词回退。之后追加状态/日期等筛选时必须保留前面的实体 scope，不能清掉实体关键词。列表里选择“最近”必须有日期列和详情入口：foreach body=[] 采集实体标识、状态、日期、detail_url，再用 data_query 在 into 表里 WHERE 实体+状态、ORDER BY date_ts DESC LIMIT 1，最后用 {q[detail_url]} 打开。不要采集 pending_orders 后又让 navigation 自己猜第一行。","goal":"处理某客户最近一笔 pending 记录","steps":[
 ...
 {"op":"run","run_kind":"filter","var":"f1","name":"在客户字段用精确值『客户全名』筛选","success_condition":"客户精确筛选已应用，records-found 计数可读（是否 0 条由返回值判读）","target_controls":["客户字段"],"target_values":{"客户字段":"客户全名"},"returns":["match_count"],"read_spec":"match_count：读取 records found 计数","return_domains":{"match_count":"number"}},
 {"op":"if","cond_var":"f1","cond_field":"match_count","cond_cmp":"==","cond_value":"0","then":[
   {"op":"run","run_kind":"filter","name":"清除精确值后在同一客户字段用关键词『客户关键词』重筛","success_condition":"客户关键词筛选已应用且匹配记录非 0 条","target_controls":["客户字段"],"target_values":{"客户字段":"客户关键词"}}]},
...
{"reasoning":"目标值依赖父实体 E 下的子记录统计，但最终 mutation 发生在父实体字段上，所以拆成两段数据源：先到子记录集合，以父实体字段筛选 E，foreach 采子记录的父实体标识和详情 URL，钻详情补统计字段，再用 data_query count；compute 只把 count 标量组装成文本。然后回父实体列表，按 E 精确→关键词检索，foreach body=[] 采父候选的 name/type/detail_url，用 data_query 按实体谓词和应用知识给出的父行区分字段选唯一父行，最后打开父行 URL 并 action 写字段保存。修复前序查询问题时必须保留最后的写入 action。","goal":"根据父实体 E 的子记录评分统计更新父实体摘要字段","steps":[
 ...
 {"op":"run","run_kind":"filter","var":"f1","name":"在父实体字段用精确值『E』筛选","success_condition":"父实体精确筛选已应用，records-found 计数可读","target_controls":["父实体字段"],"target_values":{"父实体字段":"E"},"returns":["match_count"],"read_spec":"match_count：读取 records found 计数","return_domains":{"match_count":"number"}},
 {"op":"if","cond_var":"f1","cond_field":"match_count","cond_cmp":"==","cond_value":"0","then":[
   {"op":"run","run_kind":"filter","name":"清除精确值后在同一父实体字段用关键词『K』重筛","success_condition":"父实体关键词筛选已应用且匹配记录非 0 条","target_controls":["父实体字段"],"target_values":{"父实体字段":"K"}}]},
 ...
 {"op":"run","run_kind":"filter","var":"f2","name":"在父实体名称字段用精确值『E』筛选","success_condition":"父实体精确筛选已应用，records-found 计数可读","target_controls":["父实体名称字段"],"target_values":{"父实体名称字段":"E"},"returns":["match_count"],"read_spec":"match_count：读取 records found 计数","return_domains":{"match_count":"number"}},
 {"op":"if","cond_var":"f2","cond_field":"match_count","cond_cmp":"==","cond_value":"0","then":[
   {"op":"run","run_kind":"filter","name":"清除精确值后在同一父实体名称字段用关键词『K』重筛","success_condition":"父实体关键词筛选已应用且匹配记录非 0 条","target_controls":["父实体名称字段"],"target_values":{"父实体名称字段":"K"}}]},
 {"op":"foreach","loop_var":"parent","row_fields":["parent_name","parent_kind","detail_url"],"into":"parent_rows","body":[]},
 {"op":"run","run_kind":"data_query","var":"q_parent","name":"验证并选出唯一父实体入口","returns":["match_count","detail_url"],"sql":"SELECT COUNT(*) AS match_count, CASE WHEN COUNT(*) = 1 THEN MAX(detail_url) ELSE '' END AS detail_url FROM parent_rows WHERE parent_name LIKE '%K%' AND parent_kind = 'primary'"},
 {"op":"if","cond_var":"q_parent","cond_field":"match_count","cond_cmp":"==","cond_value":"1","then":[
  ...
  "otherwise":[{"op":"finish","message":"未找到唯一父实体，无法安全更新。"}]}]}
