...
11. **「对集合里每一个都要做某事 / 读某属性」用 foreach 迭代，不要手工展开 N 步、也不要只做第一个**：当目标属性**不在列表网格的列里、只在每条记录的详情页**，或上层纠正/当前观察明确说列表缺少该属性字段时，**必须用迭代**：① 先一个 **list_read=true 的 read** 把这些行读成行对象数组（returns=每行用于定位/最终输出的字段，如 id，若最终输出字段列表里已有也可一并读）；② 用 **foreach** 对每行跑 body，把「打开 {row[id]} 详情」写成 navigation/action run，并在该 run 上直接挂 returns/read_spec 读取详情里的目标属性和最终输出字段（**若提供的表头里出现 `<列名>_url` 链接列**——如 `Action_url`/`Title_url`，由列表每行单元格的超链接折叠而来，意味着列表每行本身就链接到它的详情页——则**必须**走 URL 直达：① 的 list_read returns 里一并读出该 `_url` 列（用表头给出的**确切列名、大小写一致**，因为 `{row[..]}` 区分大小写），② 的打开步 name 写成「打开 {row[<列名>_url]}」并挂详情 returns，**不要再写 `打开 {row[id]} 详情` 去界面里逐条点开**——运行时该 URL 会被填进 name，浏览器场景确定性跳转、不走界面点击，更快更稳。仅当**没有**任何 `_url` 链接列时（或 iPhone/Android 平台），才回退到按 {row[id]} 在界面里打开）；③ foreach 自动把每行结果汇成 into 表；④ 循环结束后，才用 **data_query** 对这张 into 表做筛选/聚合，finish 引用其结果。**如果计划里出现 `FROM detail_rows` / `FROM <into表>` 或 SQL 使用详情字段（detail_score、rating 等），该表和字段必须由前面的 foreach into + body run 的 returns 真实产生；禁止跳过 foreach、凭空查询一个尚未产生的表或详情字段。**严禁**把「逐条打开详情读取再筛选」写成只处理第一条、或把若干条手工展开成一长串重复 step（行数运行时才知道、且易超长截断）。
...
示例（目标属性只在详情里 → list_read 顺手读出每行 `_url` 链接列 + foreach 用行内 URL 直达钻取 + data_query 筛选）——
{"reasoning":"任务要返回候选记录中详情页字段满足条件的负责人，但列表网格没有该详情字段，实际值只在每条记录详情里。提供的表头里有 `Action_url` 链接列（每行操作/详情列的超链接折叠而来），说明每行本身链接到它的详情页。所以：先筛出候选记录，用一个 list_read 把候选行读成行对象数组并顺手读出 id 与 Action_url，再 foreach 用 {row[Action_url]} 真实 URL 直达每条详情（浏览器确定性跳转、不逐条点界面），让该 run 返回 detail_score+owner_name(自动汇成 detail_rows 表)，循环后用 data_query 在 detail_rows 表上筛 detail_score<=3，finish 返回负责人。绝不手工展开每条、也不能只读第一条。","goal":"返回详情分数3及以下的记录负责人","steps":[
 ...
 {"op":"run","run_kind":"read","var":"r","name":"读取候选记录行的 id 与详情链接","returns":["id","Action_url"],"list_read":true,"read_spec":"id：逐行读取列表里每条候选记录所在行的 ID 列数值；Action_url：该行操作/详情列的链接（列表单元格超链接，已折叠为 Action_url 列）。每行一个对象。"},
 {"op":"foreach","loop_var":"row","over":"r","into":"detail_rows","body":[
   {"op":"run","run_kind":"navigation","var":"d","name":"打开 {row[Action_url]}","success_condition":"进入该记录详情页，显示详情分数与负责人","returns":["detail_score","owner_name"],"read_spec":"detail_score：详情里的分数/等级，判成整数；owner_name：负责人姓名。"}
 ]},
...
示例变体（列表**没有** `_url` 链接列，或 iPhone/Android → 回退按 id 在界面逐条打开；与上例只差①②两行）——
{"reasoning":"同上，但提供的表头里没有任何 `_url` 链接列（详情入口要在界面里点开），所以只 list_read 读出每行 id，foreach 的打开步按 {row[id]} 在界面里逐条打开详情。","goal":"返回详情分数3及以下的记录负责人","steps":[
