...
示例（**per-row 实体 join：每行要查的属性在【另一个关联实体】上、且该实体要靠从行字段派生的键去搜索定位 → 把这段子过程写成一个【函数】，foreach 里 op=call 逐行调用；派生用 compute 切出去**）——任务：取库存为3的产品的材质；qty 筛出的是子变体，但材质只挂在它的父配置型产品上（变体行里没有父产品的链接）：
{"reasoning":"材质在父配置型产品上、子变体为空、变体行不含父产品 URL——per-row 要从变体派生父产品键→搜→判别 Configurable 行→开→读。**用 SKU 派生父键最稳**：变体 SKU=WS08-XS-Blue，父 SKU=WS08（去掉 -SIZE-COLOR），按 SKU 搜是精确匹配；按全名搜走全文索引常 0 命中。写成函数 resolve_parent_material(sku)：compute 取父 SKU（纯计算），filter 用顶部 Search by keyword 搜父 SKU（**只写本页搜索动作，不折「回列表页」、不标 precondition——运行时会确定性地在函数体最前插入幂等的回列表到达步**），navigation 打开 Type=Configurable 行读 material。每个 GUI milestone 的 success_condition **锚定 {base}**，否则上一行残留的编辑页会满足泛化验收、漏搜。main 筛 qty=3，foreach 采 Name+SKU、每行 call 这个函数（row.SKU 绑到 sku），material 随行汇进 into 表；data_query 取非空主材质。函数只分解一次、被调多次。","goal":"取库存为3的产品的材质","functions":[
  {"name":"resolve_parent_material","params":["sku"],"returns":["material"],"body":[
    {"op":"compute","var":"base","expr":"sku.rsplit('-',2)[0]"},
    {"op":"run","run_kind":"filter","name":"在顶部 Search by keyword 输入框清空已有内容、输入父产品 SKU {base}，按回车或点放大镜提交搜索","success_condition":"Products 列表已按关键词 {base} 刷新，结果行里出现 SKU={base}、Type=Configurable Product 的父产品"},
    {"op":"run","run_kind":"navigation","var":"d","returns":["material"],"name":"点搜索结果里 SKU={base}、Type=Configurable Product 那一行的 Edit 链接，打开它的编辑页","success_condition":"已进入 SKU 为 {base} 的 Configurable 父产品编辑页，可见 Material 字段","read_spec":"material：父产品 Material 多选框第一个已选项的 value（主材质），为空则留空"}
  ]}],
 "steps":[
 ...
 {"op":"foreach","loop_var":"row","into":"product_materials","returns":["Name","SKU"],"body":[
   {"op":"call","func":"resolve_parent_material","call_args":{"sku":"{row[SKU]}"},"var":"m"}
 ]},
