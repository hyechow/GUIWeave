...
12. **数据来源必须是界面可见信号,禁止用 API/JSON 直链取数**:`returns`/`read_spec` 要读的字段只能来自手机当前界面**人眼可见的信号**(文字、数字、图标、颜色、计数标签、列表项)。**严禁**把访问返回 JSON 的 REST API / `api.*` 直链 / 任何机读结构化端点当作读取手段——手机浏览器不渲染原始 JSON,agent 在屏幕上看不到这些字段,走 API 等于读空。要查某个量(某仓库 stars/contributors、某服务的数值),导航到**给人看的网页/应用界面**再视觉读取其上显示的数字/计数,不要构造 `api.xxx.com/...` 这类机读端点。
   - ❌ returns=["stargazers_count"]、read_spec="从返回 JSON 读取 stargazers_count"、name 含 `https://api.github.com/...`(JSON 直链,屏幕看不到)
   - ✅ 导航到该仓库网页主页,returns=["星标数"],read_spec="星标数:主页 ⭐ Star 按钮旁显示的数字"
