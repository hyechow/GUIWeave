...
12. **数据来源:界面可见信号 或 经浏览器访问的 API,用 read 读取;精确数字/JSON 开 `text_source`**:`returns`/`read_spec` 要读的字段来自手机当前界面人眼可见的信号(文字、数字、图标、颜色、计数标签、列表项)。**不刻意禁止你知道的原生能力**:模型若知道某服务有 REST API(如 GitHub `api.github.com/repos/{owner}/{repo}` 返回 `stargazers_count` 等),可以**经浏览器**导航到该 endpoint 取数——但**只能在浏览器里访问**(agent 操作真实界面),不要构造绕过界面的 host 侧后门(如直接 HTTP 取 JSON,那等于偷看答案)。关键:API 返回原始 JSON、手机浏览器里视觉 OCR 读不准数字——这时给读数步骤开 **`text_source=true`**(read_screen_text):reader 改从当前帧的可见文本(a11y tree,视口内,读到的=看到的)抽取 returns 字段,而非从截图,数字/JSON 因此精确可靠。`text_source` 是只读非交互能力(不点不滚、不进每轮感知),仅在需要精确读数字/列表/API JSON 时开;普通视觉可读的字段不必开。
   - ✅ stars:导航到 `https://api.github.com/repos/google-research/android_world`,returns=["stars"],read_spec="stars=stargazers_count 的数字",**text_source=true**(从 JSON 可见文本读字段)
   - ✅ contributors:导航到该仓库网页主页,returns=["contributors"],**text_source=true**,read_spec="contributors=右侧 Contributors 区显示的数字(可数 @用户名列表)"——contributors 走 API 要分页累加、不直接,用网页可见列表更可靠
