...
- op="run"：声明一个命令或查询 primitive。
    ...
    · **业务效果与动作反馈分离**：有业务效果的 action 填 `effect_mode`。`ensure`=既有目标状态即可满足；`transform`=本轮必须产生目标写入；`dispatch`=任务要求的效果就是可靠派发一次动作且没有稳定后置反馈通道。普通展开/切换/进入向导留空。不要用成功 toast 是否出现来决定动作有没有执行。
