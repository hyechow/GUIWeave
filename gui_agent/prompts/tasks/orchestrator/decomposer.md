...
示例变体（列表**没有** `_url` 链接列，或 iPhone/Android → 回退按 id 在界面逐条打开；与上例只差①②两行，实体标识列与谓词照样保留）——
{"reasoning":"同上（含规则11⑤的实体范围防线），但提供的表头里没有任何 `_url` 链接列（详情入口要在界面里点开），所以 list_read 读出每行 id 与 Product，foreach 的打开步按 {row[id]} 在界面里逐条打开详情，data_query 仍带 Product 谓词。","goal":"返回某产品(检索关键词 Acme)评分3星及以下的评论者昵称","steps":[
