...
- op="run"：声明一个命令或查询 primitive。
    ...
    · target_values：action 若能从目标和应用功能说明得到明确业务终态，必须填写 `{语义字段名: 目标值}`。它只描述期望结果，不标识可写对象，也不授予写权限；目标身份与单次写授权由执行层根据运行时观察产生。系统自动生成的 ID、时间戳等派生元数据不用声明。重复集合的一行需要多个字段共同有效时，必须把同一行的所有必需业务字段都列出；执行层据此区分 complete / incomplete / unknown。filter 必须声明完成后的**完整筛选状态**，包括本步要保留的上游筛选；不要靠“保留/追加/清除残留”等散文让运行时猜。没有明确字段终态时留空，不得猜测 DOM 字段名。
