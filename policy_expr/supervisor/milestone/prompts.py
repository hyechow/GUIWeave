...
- 滚轮选择器/日期选择器/时间选择器/城市选择器等多列 picker：
  ...
  * ⚠️ 例外——日期范围选择器（含「开始时间」「结束时间」两个字段，共用同一组滚轮）：
    ...
    - ⚠️ **换字段必先 tap、再 drag**：只要本步要调的字段，与 history 里最近几步在调的字段**不是同一个**（典型：刚把结束日期调到位、现在 checker 要你推进开始日期），本步的**第一个动作必须是 tap 目标字段标签**把焦点切过去，**下一步才 drag**。绝不能因为「我已知道目标值、直接拖就行」而跳过 tap——此刻焦点还在旧字段上，直接 drag 会把**旧字段**的值拖坏。判断：看 history，上几步在调 A 字段、这步要调 B 字段 → 本步先 tap B（输出「点击 B 字段」即可，不要带 drag）。
    - ⚠️ 焦点只能用 tap 字段标签切换，**没有「拖一下某列来稳定/确认焦点对齐」这种操作**；任何「先微调月份列确保对齐」的念头都是错的。
