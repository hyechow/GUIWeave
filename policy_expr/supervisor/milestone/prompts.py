...
## 相对时间换算
任务中出现「本周/上周/上上周/本月/上月/今天/昨天/最近X天」等相对时间表达时，必须根据用户消息中提供的「当前日期」换算为明确的日期区间（格式：YYYY-MM-DD ~ YYYY-MM-DD），并以「时间范围：YYYY-MM-DD ~ YYYY-MM-DD」的形式加入 global_constraints。中国以周一为一周的第一天。不要把相对时间留在 goal 或 success_condition 里，必须用具体日期。

⚠️ 周运算必须用天数偏移（×7），严禁用月份偏移代替：
- 「上周」= 本周一 - 7天（起） 到 本周一 - 1天（止）
- 「上上周」= 本周一 - 14天（起） 到 本周一 - 8天（止）
- ✗ 错误：本周一 2026-05-25，上周一 = 2026-05-25 - 1个月 = 2026-04-25（月运算！错误）
- ✓ 正确：本周一 2026-05-25，上周一 = 2026-05-25 - 7天 = 2026-05-18，上周范围 2026-05-18 ~ 2026-05-24
...
9. ⚠️ 需要先筛选再采集时（如按日期区间、关键词等筛选后收集数据），筛选与采集必须拆分为独立子目标：
   - 先生成 kind=filter 子目标，验收条件为筛选已生效的可见状态（如「账单页显示指定日期范围（如 2026-05-18 至 2026-05-24）的记录」）——必须使用 global_constraints 中的实际日期，禁止照搬此处的示例日期
