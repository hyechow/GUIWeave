...
- **Admin 网格「Columns 面板启用列」子目标的验收规则** ⚠️ 高频陷阱：
  - **可靠判据**：主网格表头/行值已经出现目标列，或 Columns 面板内目标 checkbox 的 DOM/AX checked 状态为 on/true。
  - Columns 浮层会遮住部分表头；若 checkbox 已 checked/on，不能因为表头被浮层遮住就重复点击同一 checkbox（重复点击会取消勾选）。
  - **⚠️ Cancel 会撤销所有改动**（通用规则）：点 Cancel = 取消自面板打开以来的所有勾选改动，之前勾的列全部恢复原状。**正确关闭方式：点击面板外的空白区域**（关闭面板同时保留已勾选的改动）。
  - **正确反馈**：若目标 checkbox 未勾选 → 说明「需在面板内找到目标 checkbox 并点击（网格立即更新）」。若 checkbox 不在视口 → 说明「需先在面板内向下滚动找到 checkbox 再点击」。
...
- **Product Attribute 的 Text Swatch 选项验收**：
  - Manage Swatch 的一行包含多个字段，Admin swatch value 与相邻的 option/store-view label 不是同一个字段。必须按 DOM 表单清单里的同一 `group(row=...)` 及具体 `field/name` 判断。
  - 若目标值只出现在 `optiontext`/Option Label，而 `swatchtext`/Admin Swatch 仍为空或显示 `Swatch` 占位符，该选项仍不完整，必须先把目标值填入 Admin Swatch；不得因为截图中出现了目标文本就宣称选项已存在、已添加或要求直接 Save。
  - 只有该行要求的 swatch/label 字段完整后，才进入 Save Attribute 提交阶段。
