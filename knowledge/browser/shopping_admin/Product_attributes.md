...
scope:
  ...
  - planner
  - replanner
selector_when: 在 Stores > Attributes 中查找或编辑既有 catalog attribute、添加 new option、swatch value 时查阅本节
when: 在 Stores > Attributes 中查找或编辑既有 catalog attribute、添加 new option、swatch value 时查阅本节
...
`Size` is an existing product attribute whose attribute code is `size`. Adding a new size option
means:

1. Filter **Attribute Code** by `size` and submit **Search**.
2. Open the existing `size` row.
3. In that attribute's Options/Values or swatch-options area, ensure an option with the requested
   label exists; add it only when absent.
4. Save the existing attribute and verify that its option collection contains that label.
...
fields required by the form are populated). A row where the requested label appears only in a
neighboring text label while the swatch field still shows the `Swatch` placeholder is incomplete:
fill the swatch field before saving. Judge this from the row/field association in the DOM control
inventory, not from the mere presence of the label somewhere in the screenshot.

The browser control inventory exposes the two required admin-side semantics as **Admin Swatch**
and **Admin Description** (the row association is authoritative). Both fields in the same option
row must equal the requested label. **Save Attribute** only persists the row; it is not one of the
business fields that defines whether the option member is complete.

After **Add Swatch** creates a blank row, fill **Admin Description first**, then fill **Admin
Swatch**, using the requested label in both. Admin Description is required on this form: leaving
it empty prevents **Save Attribute** from persisting the new option, even when the swatch field
already contains the label. When the executor performs one field write per turn, use this order:
...
be accepted as evidence that the requested label was added. The business target is the pair
**Admin Description** = requested label and **Admin Swatch** = requested label; **Options/Values**
is only their container and **Save Attribute** is only the persistence command, not either
business target.
...
identified by both its owner (`Attribute Code=size`) and its member fields. Those member fields
are Admin Description and Admin Swatch equal to the requested label; Options/Values is their
container, and Save Attribute is the persistence command rather than a business field.
