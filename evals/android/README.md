...
| structured_read | 1 | 视觉读取原语（reader LLM 从截图读 returns 字段）。20260625_202900 github-repo：从 320 宽仓库主页截图读 stars——**reader(qwen3.5-35b) 3 次全错**（读空 / 15.2k / 142k / 32.8k，真值≈803）；放大到 640/960 反而全空（插值更糊）。强视觉模型(analyze)能从同一张读到 803 → 根因是 **reader 视觉能力不足**，非 prompt/分辨率能解；攻破需换更强 reader 模型（成本权衡）。contributors 手机网页无明确计数，标为任务本质困难 |
