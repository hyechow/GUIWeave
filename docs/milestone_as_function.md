...
对两天全部离线评测日志的 247 条 validator 重试议题分类：**阻抗类（在非代码媒介里写代码：模板表达式/
SQL 模板引用/compute 属性风格/伪语法）仅 8 条 ≈ 3%**；语义类（死值 25%、作用域 17%、foreach 字段
溯源、SQL 聚合逻辑…）≈ 74% + 未分类 23%（抽样全为 SQL 语义错）。语义错误换成 Python 书写面照样会犯,
而裸 Python 执行会把它们从「编译期重试」变成「运行时静默错答」。**结论：书写面不是瓶颈,DSL program
维持主线（07-02 拍板获得数据背书）；pysurface 继续搁置,重开判据 = 阻抗占比显著上升。**

**第三根支柱（用户补充,2026-07-06,最根本）：渐进式编排要求计划是【偏程序】（partial program）。**
三级形态（圈选 member_desc > 展开 expansion > 逐行 body_goal）全依赖计划里存在【类型化的洞】——
t=0 写语义合同,运行时拿真实行填充。Python 函数是全定义,语言里没有"这段等看到数据再写"的一等概念;
在 Python 里表达洞 = 生成带 TODO-lambda 回调 LLM 的源码 = 用更差的工效学重新发明 IR。DSL program
不是残废的 Python,是偏程序的表示法——而偏程序正是 GUI 任务的真实形态（skeleton-light 778 live 1.0
闭环建立在洞之上;member_desc 从提案到 live 便宜,正因为数据形态上加一级渐进形态 = 一个字段 + 一个
检查点调用）。三支柱：①程序即数据的四台机器（静检/采样/热补丁/服从校验）②阻抗占比 3%（书写面非瓶颈）
③偏程序（渐进式编排的构成性需求）。 诚实告警：
S10 之前 compute 方言错误不被 validator 标记（row.sku 曾静默过检、运行时才炸）,历史语料对阻抗类
略有低估——即便按观察到的 185 族样本率加倍估到 ~6-8%,仍远非主导。

## 架构自检（2026-07-06，五个方向性问题,按严重度排）

1. **恢复机制丛林,无异常体系**（最严重,含本分支自己叠的）：九种恢复/重试机制
   （validator 重试、空返回账本、域违约、kickback+adherence、非 UI kickback、程序手术、
...
2. **规则堆积无退役机制**：validator 44 条 + decomposer.md 千字 bullet,「每败例一条规则」的
   ...
   → **制度已立（见 CLAUDE.md 结构优先制度,2026-07-06）**。
3. **FFI 合同不对称**：出参类型化了,后置条件仍是散文交视觉 checker;dispatch gate 家族是
   零售特例（每种 gate = 字符串模板 + 语言绑定的 name-regex 分类器,全架构最脆层,
   186/702 修复模式=正则误分类→gate 挂错→死循环）。
   → 下一个大机制候选：后置条件结构化谓词（url_changed / active_filters== / toast_contains /
   element_visible + 散文兜底）,把 gate 家族统一成后置条件类型系统;先用 live 数据统计
   「checker 误判占失败比例」定优先级。
4. **评测断言方法论**：字面词表 vs 命名自由的生成器,两天三冤案,两次险些误导架构判断。
   → 新断言一律语义不变量（AST/数据流可判,如「存在从行 SKU 派生且锚定父搜索的 compute」）,
   旧断言按冤案逐个改。S10b 的作用域分析已示范可行性。
5. **已知结构债重申**：loop.py 上帝函数（~700 行三条控制流绞在一个 while,live 后随第 1 条
   一起状态机化）;Milestone 载体冗余（跨平台专项,但每天在收理解税）。

元问题：架构最好的部分（合同/结构/度量）与最差的部分（规则堆/gate 零售/散文判定）是同一批人
两种心态的产物——赶败例时堆规则,想清楚时立结构。制度只需一条：加规则/gate/恢复机制前,
强制先回答「能否是类型、谓词或账本的一部分」。
