...
def run(ctx):
    ...
    for product in products:
        ...
        assert new_price < state["Price"], "price must decrease"
        ctx.write(
...
def run(ctx):
    ...
    rows = ctx.query("Orders", fields=["Date"])
...
def test_validate_code_does_not_require_an_unnecessary_assertion() -> None:
    source = """
def run(ctx):
    rows = ctx.query("Orders", fields=["Status"])
...
def test_runtime_dataflow_requires_consuming_read_values() -> None:
    source = """
def run(ctx):
    row = ctx.query("Products", fields=["id"])[0]
    state = ctx.read(row, fields=["Price"])
...
def test_fixture_contract_checks_query_fields_for_matching_source() -> None:
    source = """
def run(ctx):
    rows = ctx.query("Orders", fields=["Missing"])
    assert rows, "orders are required"
    return rows
"""
...
def test_fixture_contract_requires_gui_context_for_absent_query_source() -> None:
    source = """
def run(ctx):
    rows = ctx.query("Orders", fields=["ID"])
    assert rows, "orders are required"
    return rows
"""
...
"""
    ...
    assert result.trace[0].op == "query"
    assert result.trace[0].kwargs["filters"] == {"Status": "Complete"}
...
def test_execute_code_write_updates_fixture_state() -> None:
    ...
    assert [event.op for event in result.trace] == ["query", "read", "write"]


def test_fixture_query_normalizes_unique_phone_alias() -> None:
    source = """
def run(ctx):
    rows = ctx.query("+1 205 881 2302", fields=["id", "phone"])
...
def test_fixture_semantic_fields_ignore_case_spaces_and_underscores() -> None:
    source = """
def run(ctx):
    rows = ctx.query("Orders", fields=["Grand Total (Purchased)"])
    assert rows, "orders are required"
...
def test_fixture_preserves_gui_write_evidence_when_later_code_fails() -> None:
    source = """
def run(ctx):
    product = ctx.query("Products", fields=["id"])[0]
...
def test_runtime_query_yields_lookup_then_acquire_and_resumes_python() -> None:
    source = """
def run(ctx):
    rows = ctx.query(
        "Orders",
...
"""
    ...
    assert runtime.current.statement.scope == "lookup"
    request = runtime.current.inputs["lookup_request"]
    assert request["filters"] == {"Status": "Complete"}
...
def test_runtime_read_target_remains_one_public_call_with_internal_focus() -> None:
    source = """
def run(ctx):
    state = ctx.read({"ID": "1"}, fields=["Status"])
...
"""
    ...
    llm = _SequenceLLM(
        ...
        json.dumps({
            "approve": False,
            "edits": [{
                "search": '    rows = ctx.query("Orders")',
                "replacement": '    rows = ctx.query("Orders", fields=["ID"])',
            }],
        }),
    )
