...
from gui_agent.reports.statement_reducer import fold_checklist_from_verdict
...
def test_per_item_verdicts_set_independent_status_and_evidence():
    ...
    assert all(i.source == "verdict:item" for i in cmap.values())
...
def test_missing_evidence_kept_at_true_status_not_forced_done():
    ...
    assert items["某遗留检查项未确认"].source == "verdict:missing_evidence"
