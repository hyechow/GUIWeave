...
def test_reworded_retype_is_left_to_post_action_progress_assessment():
    ...
    check = _SingleCheckResult(status="in_progress", effect_status="unverified", reason="筛选未生效", summary="进行中")
...
def test_repeated_type_history_does_not_block_submit_plan():
    ...
    history = [_typed_turn(3), _typed_turn(6)]
    obs = Observation(png_bytes=b"png", source="browser", url=URL_POSTRESET)
    check = _SingleCheckResult(status="in_progress", effect_status="unverified", reason="筛选词已输入但尚未提交", summary="进行中")
    step = p._plan_single(_ms(), check, obs, history)
    assert step.summary != STUCK
    assert step.should_act
...
def test_repeated_old_value_does_not_block_new_fallback_value():
    ...
    history = [_typed_turn(3), _typed_turn(6)]
    obs = Observation(png_bytes=b"png", source="browser", url=URL_POSTRESET)
    check = _SingleCheckResult(status="in_progress", effect_status="unverified", reason="精确词 0 条，需改用关键词 Olivia", summary="进行中")
    step = p._plan_single(_ms(), check, obs, history)
    assert step.summary != STUCK
    assert step.should_act
...
def test_same_type_template_in_different_row_scopes_is_not_flagged():
    ...
    check = _SingleCheckResult(status="in_progress", effect_status="unverified", reason="价格未保存", summary="进行中")
    step = p._plan_single(_ms(), check, obs, history)
    assert step.summary != STUCK
    assert step.should_act
