...
def test_preflight_accepts_value_role_entities_consumed_by_mutation():
    # Values to SET are not searched/iterated, but they must still be carried verbatim into the
    # mutation node. This keeps lookup coverage rules off them without allowing silent value loss.
    program = Program(statements=[
        Run(kind="navigation", name="进入 Cart Price Rules 页面"),
        Run(
            kind="action",
            name=(
                '填写 Rule Name 为 "Thanks giving sale"，'
                'Customer Groups 设为 all registered customers，保存'
            ),
        ),
    ])
